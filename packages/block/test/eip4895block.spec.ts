...
describe("EIP4895 tests", () => {
  it("should correctly generate withdrawalsRoot", async () => {
    ...
    const gethBlockBytesArray = RLP.decode(
      hexToBytes(gethWithdrawals8BlockRlp),
    );
    const withdrawals = (gethBlockBytesArray[3] as WithdrawalBytes[]).map(
      (wa) => createWithdrawalFromBytesArray(wa),
    );
    assert.equal(withdrawals.length, 8, "8 withdrawals should have been found");
    const gethWithdrawalsRoot = (
      gethBlockBytesArray[0] as Uint8Array[]
    )[16] as Uint8Array;
    assert.deepEqual(
