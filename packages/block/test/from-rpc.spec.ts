...
describe("[fromRPC]:", () => {
  it("Should create a block with JSON data that includes a transaction with value parameter as integer string", () => {
    ...
    (blockDataTransactionValueAsInteger.transactions[0] as JSONRPCTx).value =
  ...
  it("Should create a block with JSON data that includes a transaction with defaults with gasPrice parameter as integer string", () => {
    ...
    (
      blockDataTransactionGasPriceAsInteger.transactions[0] as JSONRPCTx
    ).gasPrice = gasPriceAsIntegerString;
    ...
    assert.equal(
      (
        createBlockFromTransactionGasPriceAsInteger.transactions[0] as LegacyTx
      ).gasPrice.toString(),
  ...
  it("should create a block from london hardfork", () => {
    ...
    const block = createBlockFromRPC(testdataFromRPCGoerliLondonData, [], {
      common,
    });
    assert.equal(
    ...
    assert.equal(
      ...
      testdataFromRPCGoerliLondonData.hash,
    );
  });
  ...
  it("should create a block with EIP-4896 withdrawals", () => {
    ...
    const block = createBlockFromRPC(testdataFromRPCWithWithdrawalsData, [], {
      common,
    });
  ...
  it("should create a block header with the correct hash when EIP-4896 withdrawals are present", () => {
    const common = new Common({ chain: Mainnet, hardfork: Hardfork.Shanghai });
    const block = createBlockHeaderFromRPC(testdataFromRPCWithWithdrawalsData, {
      common,
    });
...
describe("[fromRPC] - Alchemy/Infura API block responses", () => {
  ...
  it("should create pre and post merge blocks from Infura API responses to eth_getBlockByHash and eth_getBlockByNumber", () => {
    ...
    block = createBlockFromRPC(infura2000004withTransactionsData, [], {
      common,
      setHardfork: true,
    });
    assert.equal(
  ...
  it("should correctly parse a cancun block over rpc", () => {
    ...
    const block = createBlockHeaderFromRPC(infuraGoerliBlock10536893Data, {
      common,
    }); // cspell:disable-line
...
describe("[fromJSONRPCProvider]", () => {
  it("should work", async () => {
    ...
    const blockHash =
      "0x1850b014065b23d804ecf71a8a4691d076ca87c2e6fb8fe81ee20a4d8e884c24";
    const block = await createBlockFromJSONRPCProvider(provider, blockHash, {
      common,
    });
    assert.equal(
      bytesToHex(block.hash()),
    ...
    try {
      await createBlockFromJSONRPCProvider(
        provider,
