...
describe("EIP1559 tests", () => {
  ...
  it("Header -> genericFormatValidation checks", async () => {
    try {
      ...
      assert.fail(
        "should throw when baseFeePerGas is not set to initial base fee",
      );
    } catch (e: any) {
      const expectedError =
  ...
  it("Header -> validate()", async () => {
    try {
      ...
      assert.ok(
        e.message.includes("base fee"),
        "should throw on wrong initial base fee",
      );
    }
  });
      ...
      createBlockHeader(
        {
          ...
          gasUsed:
            genesis.header.gasLimit *
              (common.param("elasticityMultiplier") ?? BigInt(0)) +
            BigInt(1),
          baseFeePerGas: common.param("initialBaseFee"),
        },
        {
  ...
  it("Header -> validate() -> gasLimit -> success cases", async () => {
    ...
    assert.ok(
      true,
      "should not throw if gas limit is between bounds (HF transition block)",
    );
