...
export function createBlockFromBytesArray(
  ...
  if (
    header.common.isActivatedEIP(6800) &&
    executionWitnessBytes === undefined
  ) {
    throw new Error(
  ...
  if (header.common.isActivatedEIP(6800)) {
    if (executionWitnessBytes !== undefined) {
      executionWitness = JSON.parse(
        bytesToUtf8(RLP.decode(executionWitnessBytes) as Uint8Array),
      );
...
export async function createBlockFromExecutionPayload(
  ...
  if (
    ...
    throw Error(
      "Missing executionWitness for EIP-6800 activated executionPayload",
    );
  }
  ...
  if (
    !equalsBytes(
      block.hash(),
      hexToBytes(payload.blockHash as PrefixedHexString),
    )
  ) {
