...
export class BlockHeader {
  ...
  protected _genericFormatValidation() {
    ...
    if (this.common.isActivatedEIP(4895)) {
      if (this.withdrawalsRoot === undefined) {
        const msg = this._errorMsg(
          "EIP4895 block has no withdrawalsRoot field",
        );
        throw new Error(msg);
      }
  ...
  protected _consensusFormatValidation() {
    ...
    if (this.common.consensusAlgorithm() === ConsensusAlgorithm.Clique) {
      ...
      if (!equalsBytes(this.mixHash, new Uint8Array(32))) {
        const msg = this._errorMsg(
          `mixHash must be filled with zeros, received ${this.mixHash}`,
        );
        throw new Error(msg);
      }
    }
  ...
  public calcNextExcessBlobGas(childCommon: Common): bigint {
    ...
    const targetGasConsumed =
      (this.excessBlobGas ?? BIGINT_0) + (this.blobGasUsed ?? BIGINT_0);
