...
export class StatefulVerkleStateManager implements StateManagerInterface {
  ...
  public initVerkleExecutionWitness(
    ...
    this._debug(
      `initVerkleExecutionWitness postState=${JSON.stringify(this._postState)}`,
    );
  }
  ...
  putCode = async (address: Address, value: Uint8Array): Promise<void> => {
    if (this.DEBUG) {
      this._debug(
        `putCode address=${address.toString()} value=${short(value)}`,
      );
    }
    ...
    const chunkStems = await generateCodeStems(
      codeChunks.length,
      address,
      this.verkleCrypto,
    );
    ...
    await this._trie.put(
      ...
      chunkSuffixes.slice(
        0,
        chunkSuffixes.length <= VERKLE_CODE_OFFSET
          ? chunkSuffixes.length
          : VERKLE_CODE_OFFSET,
      ),
      codeChunks.slice(
        0,
        codeChunks.length <= VERKLE_CODE_OFFSET
          ? codeChunks.length
          : VERKLE_CODE_OFFSET,
      ),
    );
  ...
  getCode = async (address: Address): Promise<Uint8Array> => {
    ...
    const chunkSuffixes = generateChunkSuffixes(
      Math.ceil(codeSize / VERKLE_CODE_CHUNK_SIZE),
    );
    ...
    chunksByStem[0] = await this._trie.get(
      ...
      chunkSuffixes.slice(
        0,
        codeSize <= VERKLE_CODE_OFFSET ? codeSize : VERKLE_CODE_OFFSET,
      ),
    );
    ...
    for (let x = 0; x < chunks.length; x++) {
      ...
      if (x === chunks.length - 1) {
        ...
        lastChunkByteIndex =
          codeSize % VERKLE_CODE_CHUNK_SIZE || VERKLE_CODE_CHUNK_SIZE;
      }
      code.set(
  ...
  getCodeSize = async (address: Address): Promise<number> => {
    const accountBytes = (
      await this._trie.get(getVerkleStem(this.verkleCrypto, address), [
        VerkleLeafType.BasicData,
      ])
    )[0];
  ...
  getStorage = async (
    ...
    const value = await this._trie.get(storageKey.slice(0, 31), [
      storageKey[31],
    ]);
      ...
      case VerkleAccessedStateType.BasicData: {
        if (this._caches === undefined) {
          ...
          const encodedAccount =
            this._caches?.account?.get(address)?.accountRLP;
  ...
  async verifyPostState(
        ...
        if (type === VerkleAccessedStateType.BasicData) {
          ...
          this.DEBUG &&
            this._debug(
              ...
              computedValue === null
                ? null
                : decodeVerkleLeafBasicData(hexToBytes(computedValue)),
            );
        }
        this.DEBUG &&
          this._debug(
    ...
    this.DEBUG &&
      this._debug(
        `verifyPostState verifyPassed=${verifyPassed} postFailures=${postFailures}`,
      );
