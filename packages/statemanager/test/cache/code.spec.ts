...
describe("Code Cache: put and get code", () => {
  for (const type of [CacheType.LRU, CacheType.ORDERED_MAP]) {
    ...
    it(`should put code`, async () => {
      ...
      const elem = cache.get(addr);
      assert.ok(
        elem !== undefined && elem.code && equalsBytes(elem.code, code),
      );
    });
...
describe("Code Cache: checkpointing", () => {
  for (const type of [CacheType.LRU, CacheType.ORDERED_MAP]) {
    ...
    it(`should revert to correct state`, async () => {
      ...
      assert.ok(
        elem !== undefined && elem.code && equalsBytes(elem.code, code2),
      );
      ...
      elem = cache.get(addr);
      assert.ok(
        elem !== undefined && elem.code && equalsBytes(elem.code, code1),
      );
    });
    ...
    it(`cache clearing`, async () => {
      ...
      assert.equal(
        cache.size(),
        0,
        "should delete cache objects with clear=true",
      );
    });
  }
});
