...
describe("RPC State Manager initialization tests", async () => {
  vi.mock("@zondjs/util", async () => {
    ...
    return {
      ...
      fetchFromProvider: vi
        .fn()
        .mockImplementation(async (url, { method, params }: any) => {
  ...
  it("should work", () => {
    ...
    assert.ok(
      ...
      "was able to instantiate state manager",
    );
    assert.equal(
      state["_blockTag"],
      "0x1",
...
describe("RPC State Manager API tests", () => {
  it("should work", async () => {
    ...
    assert.ok(
      retrievedVitalikAccount.nonce > 0n,
      "Vitalik.eth is stored in cache",
    );
    const address = createAddressFromString(
      "0xccAfdD642118E5536024675e776d32413728DD07",
    );
    const proof = await getRPCStateProof(state, address);
    const proofBuf = proof.accountProof.map((proofNode) =>
      hexToBytes(proofNode),
    );
    const doesThisAccountExist = await verifyMerkleProof(
      address.bytes,
      proofBuf,
      {
        useKeyHashing: true,
    ...
    assert.ok(
      ...
      "getAccount returns undefined for non-existent account",
    );

    assert.ok(
      state.getAccount(vitalikDotEth) !== undefined,
      "vitalik.eth does exist",
    );
    ...
    assert.ok(
      UNIContractCode.length > 0,
      "was able to retrieve UNI contract code",
    );
    ...
    assert.ok(
      storageSlot.length > 0,
      "was able to retrieve storage slot 1 for the UNI contract",
    );

    await expect(async () => {
      await state.getStorage(
        UniswapERC20ContractAddress,
    ...
    assert.ok(
      equalsBytes(slotValue, utf8ToBytes("abcd")),
      "should retrieve slot 2 value",
    );
    ...
    try {
      await state.getAccount(
        createAddressFromString("0x9Cef824A8f4b3Dc6B7389933E52e47F010488Fc8"),
      );
    } catch (err) {
    ...
    assert.deepEqual(
      {},
      clearedStorage,
      "storage cache should be empty after clear",
    );
...
describe("runBlock test", () => {
  it("should work", async () => {
    ...
    try {
      ...
      assert.fail(
        `should have successfully ran block; got error ${err.message}`,
      );
    }
  });
});
...
describe("blockchain", () =>
  it("uses blockhash", async () => {
    const blockchain = new RPCBlockChain(
      provider,
    ) as unknown as EVMMockBlockchainInterface;
    const blockTag = 1n;
    ...
    await evm.stateManager.setStateRoot(
      hexToBytes(
        "0xf8506f559699a58a4724df4fcf2ad4fd242d20324db541823f128f5974feb6c7",
      ),
    );
    const block = await createBlockFromJSONRPCProvider(provider, 500000n, {
      setHardfork: true,
    });
...
describe("Should return same value as MerkleStateManager when account does not exist", () => {
  it("should work", async () => {
    ...
    const account0 = await rpcState.getAccount(
      new Address(hexToBytes(`0x${"01".repeat(20)}`)),
    );
    const account1 = await defaultState.getAccount(
      new Address(hexToBytes(`0x${"01".repeat(20)}`)),
    );
    assert.equal(
      account0,
