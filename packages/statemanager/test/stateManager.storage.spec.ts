    ...
    for (const prefixStorageTrieKeys of [false, true]) {
      ...
      it(`should throw on storage values larger than 32 bytes`, async () => {
        ...
        try {
          ...
          assert.ok(
            true,
            "threw on trying to set storage values larger than 32 bytes",
          );
        }
      });
      ...
      it(`should strip zeros of storage values`, async () => {
        ...
        assert.ok(
          equalsBytes(slot0, expect0),
          "value of 31 bytes padded correctly",
        );
        ...
        assert.ok(
          equalsBytes(slot1, expect1),
          "value of 1 byte padded correctly",
        );
      });
      ...
      it(`should not strip trailing zeros`, async () => {
        ...
        assert.ok(
          equalsBytes(contractValue, expect),
