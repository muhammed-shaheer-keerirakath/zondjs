...
describe("StatelessVerkleStateManager: Kaustinen Verkle Block", () => {
  ...
  it("initPreState()", async () => {
    ...
    assert.ok(
      Object.keys(stateManager["_state"]).length !== 0,
  ...
  it.skip("getAccount()", async () => {
    ...
    assert.equal(
      account!._storageRoot,
      null,
      "stateroot should have not been set",
    );
    assert.equal(
  ...
  it("put/delete/modify account", async () => {
    ...
    try {
      ...
      assert.fail(
        "should throw on getting account that is not found in witness",
      );
    } catch (e: any) {
      assert.equal(
  ...
  test.skip("get/put/clear contract storage", async () => {
    ...
    let contractStorage = await stateManager.getStorage(
      contractAddress,
      hexToBytes(storageKey),
    );
    ...
    contractStorage = await stateManager.getStorage(
      contractAddress,
      hexToBytes(storageKey),
    );
