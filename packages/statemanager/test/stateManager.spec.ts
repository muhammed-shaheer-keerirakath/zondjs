...
describe("StateManager -> General", () => {
  ...
  it("should not throw on getContractStorage() on non-existing accounts", async () => {
    ...
    try {
      const storage = await sm.getStorage(
        createZeroAddress(),
        new Uint8Array(32),
      );
  ...
  it(`copy()`, async () => {
    ...
    assert.equal(
      ...
      0,
      "should set trie cache size to 0",
    );
  ...
  it(`should import proofs from getProof`, async () => {
    ...
    for (const [addressStr, entry] of Object.entries(stateSetup)) {
      ...
      stateSetup[addressStr].codeHash = (
        await stateManager.getAccount(address)!
      )?.codeHash;
      stateSetup[addressStr].storageRoot = (
        await stateManager.getAccount(address)!
      )?.storageRoot;
    }
    ...
    let stSlot1_0 = await partialStateManager.getStorage(
      address1,
      state1.keys[0],
    );
    assert.ok(equalsBytes(stSlot1_0, state1.values[0]));

    let stSlot1_1 = await partialStateManager.getStorage(
      address1,
      state1.keys[1],
    );
    assert.ok(equalsBytes(stSlot1_1, state1.values[1]));

    let stSlot1_2 = await partialStateManager.getStorage(
      address1,
      state1.keys[2],
    );
    ...
    const newPartialStateManager = await fromMerkleStateProof([
      proof2,
      stProof,
    ]);
    ...
    try {
      ...
      assert.ok(
        e.message.includes("proof does not have the expected trie root"),
      );
    }

    await addMerkleStateProofData(newPartialStateManager2, [proof2, stProof]);
    await newPartialStateManager2.setStateRoot(
      await partialStateManager.getStateRoot(),
    );
    ...
    await stateManager.putAccount(
      createZeroAddress(),
      new Account(zeroAddressNonce),
    );
    const zeroAddressProof = await getMerkleStateProof(
      stateManager,
      createZeroAddress(),
    );
    ...
    let zeroAccount =
      await newPartialStateManager2.getAccount(createZeroAddress());
  ...
  it.skipIf(isBrowser() === true)(
    ...
    async () => {
      ...
      const pk = hexToBytes(
        "0x9f12aab647a25a81f821a5a0beec3330cd057b2346af4fb09d7a807e896701ea",
      );
      const pk2 = hexToBytes(
        "0x8724f27e2ce3714af01af3220478849db68a03c0f84edf1721d73d9a6139ad1c",
      );
      ...
      assert.deepEqual(
        intToBytes(32),
        await partialSM.getStorage(address, hexToBytes(keys[0])),
      );
      ...
      assert.deepEqual(
        intToBytes(32),
        await partialSM2.getStorage(address, hexToBytes(keys[0])),
      );
