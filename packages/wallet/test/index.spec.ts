...
const fixtureEthersWallet = new ethersWallet(
...
describe("Wallet tests", () => {
  ...
  it(".getPublicKey()", () => {
    assert.deepEqual(
      bytesToHex(fixtureWallet.getPublicKey()),
      fixturePublicKey,
    );
  }, 30000);
  ...
  it(".verifyPublicKey()", () => {
    assert.deepEqual(
      fixtureWallet.verifyPublicKey(fixturePublicKeyBytes),
      true,
    ),
    ...
    assert.throws(
      function () {
        Wallet.fromPublicKey(
          hexToBytes(
            "0x030639797f6cc72aea0f3d309730844a9e67d9f1866e55845c5f7e0ab48402973d",
          ),
        );
      },
      "Invalid public key",
  ...
  it(".generate()", () => {
    assert.equal(
      Wallet.generate().getPrivateKey().length,
      32,
  ...
  const pw = "testtest";
  const salt =
    "0xdc9e4a98886738bd8aae134a1f89aaa5a502c3fbd10e336136d4d5fe47448ad6";
  ...
  it(".toV3(): should work with Scrypt", async () => {
    ...
    for (const perm of permutations) {
      ...
      const encFixtureEthersWallet = encryptKeystoreJsonSync(
        fixtureEthersWallet,
        pw,
        {
          scrypt: { N: n, r, p },
      ...
      assert.deepEqual(
        JSON.parse(encRandomWallet),
        JSON.parse(encEthersWallet),
      );
    }
  }, 180000);
  ...
  it(".toV3(): without providing options", async () => {
    ...
    try {
      ...
      assert.ok(
        err.message.includes("Unsupported kdf"),
  ...
  it("should fail for bad salt", async () => {
    const pw = "test";
    const errStr =
      "Invalid salt, string must be empty or a non-zero even number of hex characters";
  ...
  it(".toV3(): should fail for bad iv", async () => {
    ...
    try {
      await fixtureWallet.toV3(pw, {
        iv: "ffffffffffffffffffffffffffffffffff",
      });
    } catch (err: any) {
      assert.ok(err.message.includes(errStrLength));
    }
  ...
  it(".toV3(): should fail for bad uuid", async () => {
    ...
    try {
      await fixtureWallet.toV3(pw, {
        uuid: "xfffffffffffffffffffffffffffffff",
      });
    } catch (err: any) {
      assert.ok(err.message.includes(errStrLength));
    }
    try {
      await fixtureWallet.toV3(pw, {
        uuid: "fffffffffffffffffffffffffffffffx",
      });
    } catch (err: any) {
      assert.ok(err.message.includes(errStrLength));
    }
    try {
      await fixtureWallet.toV3(pw, {
        uuid: "fffffffffffffffxffffffffffffffff",
      });
    } catch (err: any) {
      assert.ok(err.message.includes(errStrLength));
    }
  ...
  describe(".fromV3()", () => {
    it("should work with PBKDF2", async () => {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        (
          await ethersWallet.fromEncryptedJson(wEthersCompat, pw)
        ).address.toLowerCase(),
      );
      assert.deepEqual(
        walletRandom.getAddressString(),
        (
          await ethersWallet.fromEncryptedJson(wRandom, pw)
        ).address.toLowerCase(),
      );
    }, 30000);
    ...
    it.skip("should work with Scrypt", async () => {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        (
          await ethersWallet.fromEncryptedJson(sample, pw)
        ).address.toLowerCase(),
      );
      assert.deepEqual(
        walletRandom.getAddressString(),
        (
          await ethersWallet.fromEncryptedJson(sampleRandom, pw)
        ).address.toLowerCase(),
      );
    });
    it.skip("should work with 'unencrypted' wallets", async () => {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        "0xa9886ac7489ecbcbd79268a79ef00d940e5fe1f2",
      );
    });
    ...
    it("shouldn't work with (broken) mixed-case input files in strict mode", async () => {
      ...
      try {
        ...
        assert.ok(
          err !== undefined,
          "threw error for broken input in strict mode",
        );
      }
    }, 30000);
    ...
    it("should fail for wrong prf in pbkdf2", async () => {
      const w =
        '{"crypto":{"kdf":"pbkdf2","kdfparams":{"prf":"invalid"}},"version":3}';
      try {
        await Wallet.fromV3(w, "testpassword");
      } catch (err: any) {
  ...
  describe(".fromEthSale()", () => {
    ...
    it("should work with short password (8 characters)", async function () {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        "0x22f8c5dd4a0a9d59d580667868df2da9592ab292",
      );
    }, 30000);
    it("should work with long password (19 characters)", async function () {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        "0xc90b232231c83b462723f473b35cb8b1db868108",
      );
    }, 30000);
    ...
    it("should work with pyethrecover's wallet", async function () {
      ...
      assert.deepEqual(
        wallet.getAddressString(),
        "0x83b6371ba6bd9a47f82a7c4920835ef4be08f47b",
      );
    }, 30000);
  });
  ...
  describe(".fromEtherWallet()", () => {
    ...
    it("should work with encrypted input", async function () {
      ...
      const wallet = await Thirdparty.fromEtherWallet(
        etherWalletEncrypted,
        "testtest",
      );
      assert.deepEqual(
        wallet.getAddressString(),
        "0x9d6abd11d36cc20d4836c25967f1d9efe6b1a27c",
      );
    }, 30000);
  });
  ...
  it(".fromQuorumWallet()", () => {
    const wallet = Thirdparty.fromQuorumWallet(
      "testtesttest",
      "ethereumjs-wallet",
    );
    assert.equal(
      wallet.getAddressString(),
      "0x1b86ccc22e8f137f204a41a23033541242a48815",
    );
  }, 30000);
