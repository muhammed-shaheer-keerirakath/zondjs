...
export class CliqueConsensus implements Consensus {
  ...
  private cliqueCheckRecentlySigned(header: BlockHeader): boolean {
    ...
    const seen = signers.filter((s) =>
      s[1].equals(cliqueSigner(header)),
    ).length;
    return seen > 1;
  }
  ...
  private async getCliqueLatestSignerStates(): Promise<CliqueLatestSignerStates> {
    ...
    const states = RLP.decode(signerStates as Uint8Array) as [
      Uint8Array,
      Uint8Array[],
    ];
    return states.map((state) => {
      const blockNum = bytesToBigInt(state[0] as Uint8Array);
      const addresses = (<any>state[1]).map(
        (bytes: Uint8Array) => new Address(bytes),
      );
  ...
  async cliqueSignerInTurn(
    ...
    return (
      (number + BigInt(1)) % BigInt(signers.length) === BigInt(signerIndex)
    );
  }
}
