...
describe("blockchain test", () => {
      ...
      async (block: Block, reorg: boolean) => {
        ...
        if (reorged === 0) {
          if (block.header.number === BigInt(10)) {
            ...
            for (let j = 5; j < 26; j++) {
              const reorgedBlock = generateConsecutiveBlock(
                reorgParent,
                1,
                BigInt(8000001),
              );
          ...
          if (
            equalsBytes(
              block.hash(),
              reorgedBlocks[Number(block.header.number) - 5].hash(),
            )
          ) {
            servedReorged++;
          }
  ...
  it("should test setIteratorHead method", async () => {
    ...
    assert.deepEqual(
      headHash,
      currentHeadBlock.hash(),
      "head hash equals the provided head hash",
    );
