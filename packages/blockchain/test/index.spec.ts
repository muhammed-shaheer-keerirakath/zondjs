...
describe("blockchain test", () => {
  ...
  it("should initialize holesky correctly", async () => {
    ...
    assert.deepEqual(
      ...
      holeskyHash,
      "correct genesis hash for holesky",
    );
  });
  ...
  it("should add 12 blocks, one at a time", async () => {
    ...
    const addNextBlock = async (number: number) => {
      ...
      if (blocks.length < 12) {
        ...
        const getBlocks = await blockchain.getBlocks(
          blocks[0].hash(),
          12,
          0,
          false,
  ...
  it("should get 5 consecutive blocks, starting from genesis hash", async () => {
    ...
    assert.equal(
      ...
      BigInt(4),
      "block 4 should be new canonical header",
    );
  ...
  it("should get 5 consecutive blocks, starting from block 5, reversed", async () => {
    ...
    assert.equal(getBlocks![0].header.number, blocks[5].header.number);
    assert.ok(
      isConsecutive(getBlocks!.reverse()),
  ...
  it("should add fork header and reset stale heads", async () => {
    ...
    assert.deepEqual(
      blockchain._heads["staleTest"],
      blocks[14].hash(),
      "should update stale head",
    );
    assert.deepEqual(
      blockchain._headBlockHash,
      blocks[14].hash(),
  ...
  it("should test nil bodies / throw", async () => {
    ...
    const block2Header = createBlockHeaderFromBytesArray(
      block2HeaderValuesArray,
      {
  ...
  it("should add block with body", async () => {
    ...
    const genesisRlp = hexToBytes(
      preLondonData.genesisRLP as PrefixedHexString,
    );
    ...
    const blockRlp = hexToBytes(
      preLondonData.blocks[0].rlp as PrefixedHexString,
    );
  ...
  it("uncached db ops", async () => {
    ...
    const td = await (<any>blockchain).getTotalDifficulty(
      genesis.hash(),
      BigInt(0),
    );
    assert.equal(
      ...
      "should perform getTotalDifficulty correctly",
    );
  });
  ...
  it("mismatched chains", async () => {
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      if (i === 2) {
        assert.ok(
          error.message.match("Chain mismatch"),
          "should return chain mismatch error",
        );
      } else {
...
describe("initialization tests", () => {
  ...
  it("should not allow to change the genesis block in the database", async () => {
    ...
    if (equalsBytes(otherGenesisBlock.hash(), hash)) {
      assert.fail(
        "other genesis block should have a different hash than the genesis block",
      );
    }
    ...
    try {
      ...
      assert.fail(
        "creating blockchain with different genesis block than in db did not throw",
      );
    } catch (e: any) {
      assert.equal(
        e.message,
...
it("should correctly derive mainnet genesis block hash and stateRoot", async () => {
  ...
  assert.deepEqual(
    blockchain.genesisBlock.header.stateRoot,
    mainnetGenesisStateRoot,
  );
});
