  ...
  it("should correctly reorg a poa chain and remove blocks from clique snapshots", async () => {
    ...
    assert.ok(
      !signerStates.find(
        (s: any) =>
          s[0] === BigInt(2) &&
          s[1].find((a: Address) => a.equals(beneficiary1)),
      ),
      "should not find reorged signer state",
    );

    let signerVotes = (blockchain.consensus as CliqueConsensus)
      ._cliqueLatestVotes;
    assert.ok(
      !signerVotes.find(
        (v: any) =>
    ...
    let blockSigners = (blockchain.consensus as CliqueConsensus)
    ...
    signerStates = (blockchain.consensus as CliqueConsensus)
      ._cliqueLatestSignerStates;
    assert.ok(
      !!signerStates.find(
        (s: any) =>
          s[0] === BigInt(3) &&
          s[1].find((a: Address) => a.equals(beneficiary2)),
      ),
    ...
    blockSigners = (blockchain.consensus as CliqueConsensus)
      ._cliqueLatestBlockSigners;
    assert.ok(
      !!blockSigners.find(
        (s: any) =>
