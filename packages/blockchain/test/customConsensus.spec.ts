...
class fibonacciConsensus implements Consensus {
  ...
  validateDifficulty(_header: BlockHeader): Promise<void> {
    ...
    if (_header.difficulty !== _header.number - 1n + _header.number - 2n) {
      throw new Error(
        "invalid difficulty - must be sum of previous two block numbers",
      );
    }
    return new Promise<void>((resolve) => resolve());
  }
...
describe("consensus transition checks", () => {
  it("should transition correctly", async () => {
    ...
    try {
      ...
      assert.ok(
        "checkAndTransitionHardForkByNumber does not throw with custom consensus",
      );
    } catch (err: any) {
      assert.fail(
    ...
    try {
      ...
      assert.ok(
        err.message.includes("Consensus object for ethash must be passed"),
      );
    }
  });
});
