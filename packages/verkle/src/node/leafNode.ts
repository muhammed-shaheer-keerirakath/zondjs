...
export class LeafVerkleNode extends BaseVerkleNode<VerkleNodeType.Leaf> {
  ...
  static async create(
    ...
    for (let x = 0; x < 256; x++) {
      if (!equalsBytes(c1Values[x], new Uint8Array(32))) {
        c1 = verkleCrypto.updateCommitment(
          c1,
          x,
          new Uint8Array(32),
          c1Values[x],
      ...
      if (!equalsBytes(c2Values[x], new Uint8Array(32))) {
        c2 = verkleCrypto.updateCommitment(
          c2,
          x,
          new Uint8Array(32),
          c2Values[x],
  ...
  static fromRawNode(
    ...
    const values = rawNode
      ...
      .map((el) =>
        el.length === 0 ? 0 : equalsBytes(el, createZeroesLeafValue()) ? 1 : el,
      );
    return new LeafVerkleNode({
      stem,
      values,
  ...
  setValue(index: number, value: Uint8Array | LeafVerkleNodeValue): void {
    ...
    const cValues =
      index < 128
        ? createCValues(this.values.slice(0, 128))
        : createCValues(this.values.slice(128));
