...
export class VerkleTree {
  ...
  async put(
    ...
    if (foundPath.node !== null) {
      ...
      this.DEBUG &&
        this.debug(`Creating new leaf node at stem: ${bytesToHex(stem)}`, [
          "put",
        ]);
    }
    for (let i = 0; i < values.length; i++) {
      ...
      if (
        ...
        equalsBytes(value, createZeroesLeafValue())
      ) {
    ...
    if (leafNode.values.every((val) => val === LeafVerkleNodeValue.Untouched)) {
      ...
      if (foundPath.node !== null) {
        ...
        this.DEBUG &&
          this.debug(`Deleting leaf node at stem: ${bytesToHex(stem)}`, [
            "put",
          ]);
    ...
    if (foundPath.stack.length > 1) {
      ...
      const updatedParentTuple = await this.updateParent(
        leafNode,
        nearestNode,
        lastPath,
      );
      if (updatedParentTuple !== undefined) {
        putStack.push([
          updatedParentTuple.node.hash(),
          updatedParentTuple.node,
        ]);
      ...
      while (foundPath.stack.length > 1) {
        const [nextNode, nextPath] = foundPath.stack.pop()! as [
          InternalVerkleNode,
          Uint8Array,
        ];
        ...
        if (childReference !== null) {
          ...
          if (
            equalsBytes(nextNode.commitment, this.verkleCrypto.zeroCommitment)
          ) {
    ...
    this.DEBUG &&
      this.debug(`Updating root node hash to ${bytesToHex(this._root)}`, [
        "put",
      ]);
  ...
  async del(stem: Uint8Array, suffixes: number[]): Promise<void> {
    ...
    await this.put(
      stem,
      suffixes,
      new Array(suffixes.length).fill(createZeroesLeafValue()),
    );
  }
  /**
  ...
  async findPath(key: Uint8Array): Promise<Path> {
    this.DEBUG &&
      this.debug(`Path (${key.length}): [${bytesToHex(key)}]`, ["find_path"]);
    const result: Path = {
      node: null,
      stack: [],
      remaining: key,
    ...
    const rootNode = decodeVerkleNode(
      rawNode,
      this.verkleCrypto,
    ) as InternalVerkleNode;
    ...
    if (child === null) {
      this.DEBUG &&
        this.debug(`Partial Path ${intToHex(key[0])} - found no child.`, [
          "find_path",
        ]);
      return result;
    }
    ...
    while (!finished) {
      ...
      if (
        foundNode ||
        child!.path.length >= key.length ||
        isLeafVerkleNode(decodedNode)
      ) {
        ...
        const pathToNearestNode = isLeafVerkleNode(decodedNode)
          ? decodedNode.stem
          : child!.path;
        this.DEBUG &&
          this.debug(
