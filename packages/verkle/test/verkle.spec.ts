...
describe("Verkle tree", () => {
  ...
  it("should insert and retrieve values", async () => {
    ...
    assert.ok(
      res.node === null,
      "should not find a node when the key is not present",
    );
    assert.deepEqual(res.remaining, presentKeys[0]);

    for (let i = 0; i < presentKeys.length; i++) {
      await tree.put(
        presentKeys[i].slice(0, 31),
        [presentKeys[i][31]],
        [values[i]],
      );
    }
    const stem = presentKeys[0].slice(0, 31);
    const retrievedValues = await tree.get(stem, [
      presentKeys[0][31],
      presentKeys[1][31],
    ]);
    ...
    for (let i = 2; i < presentKeys.length; i++) {
      const retrievedValue = await tree.get(presentKeys[i].slice(0, 31), [
        presentKeys[i][31],
      ]);
  ...
  it("should find the path to various leaf nodes", async () => {
    ...
    assert.deepEqual(
      val2[0],
      values[2],
      "confirm values[2] can be retrieved from trie",
    );
  });
  ...
  it("should sequentially put->find->delete->put values", async () => {
    ...
    assert.deepEqual(
      (await trie.get(keys[0].slice(0, 31), [keys[0][31]]))[0],
      values[0],
    );
    assert.deepEqual(
      (await trie.get(keys[2].slice(0, 31), [keys[2][31]]))[0],
      values[2],
    );
    assert.deepEqual(
      (await trie.get(keys[3].slice(0, 31), [keys[3][31]]))[0],
      values[3],
    );
  ...
  it("should put zeros in leaf node when del called with stem that was not in the trie before", async () => {
    ...
    assert.deepEqual(
      (res.node as LeafVerkleNode).values[keys[0][31]],
      LeafVerkleNodeValue.Deleted,
    );
  });
