...
describe("verkle node - leaf", () => {
  it("constructor should create an leaf node", async () => {
    ...
    assert.ok(
      equalsBytes(node.c1 as unknown as Uint8Array, c1),
      "c1 should be set",
    );
    assert.ok(
      equalsBytes(node.c2 as unknown as Uint8Array, c2),
      "c2 should be set",
    );
    assert.ok(equalsBytes(node.stem, stem), "stem should be set");
    assert.ok(
      values.every((value, index) =>
        equalsBytes(value, node.values[index] as Uint8Array),
      ),
  ...
  it("should create a leafnode with default values", async () => {
    ...
    assert.deepEqual(
      node.getValue(0),
      setLengthRight(Uint8Array.from([5]), 32),
    );
  ...
  it("should serialize and deserialize a node from raw values", async () => {
    ...
    assert.deepEqual(
      defaultNode,
      decodeVerkleNode(defaultNode.serialize(), verkle),
    );
  });
});
