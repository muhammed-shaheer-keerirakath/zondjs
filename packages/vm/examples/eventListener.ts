...
const main = async () => {
  ...
  vm.events.on("afterTx", (event, resolve) => {
    console.log(
      "asynchronous listener to afterTx",
      bytesToHex(event.transaction.hash()),
    );
  ...
  vm.events.on("afterTx", (event) => {
    console.log(
      "synchronous listener to afterTx",
      bytesToHex(event.transaction.hash()),
    );
  });
  ...
  const tx = createLegacyTx({
    ...
    v: BigInt(37),
    r: BigInt(
      "62886504200765677832366398998081608852310526822767264927793100349258111544447",
    ),
    s: BigInt(
      "21948396863567062449199529794141973192314514851405455194940751428901681436138",
    ),
  });
