    ...
    sources: {
      "helpers/Greeter.sol": {
        content: readFileSync(
          path.join(__dirname, "helpers", "Greeter.sol"),
          "utf8",
        ),
      },
...
async function deployContract(
  ...
  const tx = createLegacyTx(buildTransaction(txData as any), { common }).sign(
    senderPrivateKey,
  );
...
async function getGreeting(vm: VM, contractAddress: Address, caller: Address) {
  const sigHash = new Interface(["function greet()"]).getFunction(
    "greet",
  )!.selector;
