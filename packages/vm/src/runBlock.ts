...
const stateRootCPLabel =
  "New state root, DAO HF, checkpoints, block validation";
const processTxsLabel =
  "Tx processing [ use per-tx profiler for more details ]";
const withdrawalsRewardsCommitLabel =
  "Withdrawals, Rewards, EVM journal commit";
...
export async function runBlock(
  ...
  if (vm.common.isActivatedEIP(6800)) {
    ...
    stateManager.initVerkleExecutionWitness!(
      block.header.number,
  ...
  if (generateFields) {
    ...
    if (!(vm.stateManager instanceof StatelessVerkleStateManager)) {
      ...
      if (
        equalsBytes(result.receiptsRoot, block.header.receiptTrie) === false
      ) {
        if (vm.DEBUG) {
          debug(
    ...
    if (vm.common.isActivatedEIP(6800)) {
      if (vm.evm.verkleAccessWitness === undefined) {
        throw Error(
          `verkleAccessWitness required if verkle (EIP-6800) is activated`,
        );
      }
      // If verkle is activated and executing statelessly, only validate the post-state
      if (
        (await vm["_opts"].stateManager!.verifyPostState!(
...
async function applyBlock(
  ...
  if (
    vm.common.isActivatedEIP(6800) &&
    vm.evm.systemVerkleAccessWitness !== undefined
  ) {
...
export async function accumulateParentBlockHash(
  ...
  async function putBlockHash(vm: VM, hash: Uint8Array, number: bigint) {
    ...
    if (vm.common.isActivatedEIP(6800)) {
      ...
      vm.evm.systemVerkleAccessWitness.writeAccountStorage(
        historyAddress,
        ringKey,
      );
    }
...
export async function accumulateParentBeaconBlockRoot(
  ...
  const historicalRootsLength = BigInt(
    vm.common.param("historicalRootsLength"),
  );
