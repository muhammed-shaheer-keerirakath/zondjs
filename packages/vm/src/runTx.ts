...
async function _runTx(vm: VM, opts: RunTxOpts): Promise<RunTxResult> {
  ...
  if (vm.common.isActivatedEIP(6800)) {
    if (vm.evm.verkleAccessWitness === undefined) {
      throw Error(
        `Verkle access witness needed for execution of verkle blocks`,
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Verkle State Manager needed for execution of verkle blocks`,
      );
    }
    stateAccesses = vm.evm.verkleAccessWitness;
    txAccesses = new VerkleAccessWitness({
      verkleCrypto: vm.stateManager.verkleCrypto,
    });
  }
