 ...
 * @returns
 */
function setupCommonWithNetworks(
  network: string,
  ttd?: number,
  timestamp?: number,
  kzg?: KZG,
  ...
  const hfName = normalHardforks.reduce((previousValue, currentValue) =>
    currentValue.toLowerCase() === networkLowercase
      ? currentValue
      : previousValue,
  );
...
export function getCommon(network: string, kzg?: KZG): Common {
  ...
  if (
    normalHardforks.map((str) => str.toLowerCase()).includes(networkLowercase)
  ) {
      ...
      network in transitionNetworks
        ...
        : transitionNetworks[
            (network.charAt(0).toUpperCase() +
              network.slice(1)) as keyof typeof transitionNetworks
          ];
    ...
    for (const hf of hardforks) {
      if (mainnetCommon.gteHardfork(hf.name)) {
        // this hardfork should be activated at block 0
        const forkBlockNumber = transitionForks[
          hf.name as keyof typeof transitionForks
