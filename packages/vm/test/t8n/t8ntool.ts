...
export class TransitionTool {
  ...
  private constructor(args: T8NOptions) {
    ...
    this.txsData = normalizeTxData(
      JSON.parse(readFileSync(args.input.txs).toString()),
    );
    this.inputEnv = normalizeNumbers(
      JSON.parse(readFileSync(args.input.env).toString()),
    );
  ...
  private async run(args: T8NOptions) {
    ...
    const parentBlockHeader = makeParentBlockHeader(this.inputEnv, {
      common: this.common,
    });
  ...
  private async setup(args: T8NOptions) {
    ...
    if (args.log === true) {
      ...
      this.vm.events.on("afterTx", () => {
        // eslint-disable-next-line no-console
        console.log(
          "Done processing transaction (system operations might follow next)",
        );
      });
