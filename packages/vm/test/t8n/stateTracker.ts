...
export class StateTracker {
  ...
  constructor(vm: VM, alloc: T8NAlloc) {
    ...
    vm.stateManager.putStorage = async function (
      ...args: [Address, Uint8Array, Uint8Array]
    ) {
      const address = args[0];
      const key = args[1];
  ...
  public async dumpAlloc() {
    ...
    for (const addressString in this.allocTracker) {
      ...
      outputAlloc[addressString].code = bytesToHex(
        await this.vm.stateManager.getCode(address),
      );

      const storage = this.allocTracker[addressString].storage;
      outputAlloc[addressString].storage =
        outputAlloc[addressString].storage ?? {};
      ...
      for (const key of storage) {
        ...
        const value = await this.vm.stateManager.getStorage(
          address,
          setLengthLeft(keyBytes, 32),
        );
        if (value.length === 0) {
        ...
        outputAlloc[addressString].storage![storageKeyTrimmed] =
          bytesToHex(value);
      }
    }
    return outputAlloc;
