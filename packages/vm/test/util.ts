...
export function verifyAccountPostConditions(
  ...
  return new Promise<void>((resolve) => {
    ...
    if (
      !equalsBytes(
        format(account.balance, true),
        format(acctData.balance, true),
      )
    ) {
      t.comment(
    ...
    if (
      !equalsBytes(format(account.nonce, true), format(acctData.nonce, true))
    ) {
      t.comment(
    ...
    for (const key in acctData.storage) {
      hashedStorage[
        bytesToHex(
          keccak256(
            setLengthLeft(hexToBytes(isHexString(key) ? key : `0x${key}`), 32),
          ),
        )
    ...
    rs.on("data", function (data: any) {
      ...
      if (key === "0x") {
        ...
        acctData.storage["0x00"] =
          acctData.storage["0x00"] ?? acctData.storage["0x"];
  ...
  readAccounts(state).then(async function (accounts: any) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      for (const storageKey in results[i].storage) {
        console.log(
          "\t\t" + storageKey + ": " + results[i].storage[storageKey],
        );
      }
  ...
  if (txData.authorizationList !== undefined) {
    ...
    for (const signature of txData.authorizationList) {
      ...
      if (signature.chainId !== undefined) {
        signature.chainId = bytesToHex(
          unpadBytes(hexToBytes(signature.chainId)),
        );
      }
  ...
  return new Promise<void>((resolve) => {
    ...
    for (const key in testData) {
      const hash = bytesToHex(
        keccak256(hexToBytes(isHexString(key) ? key : `0x${key}`)),
      );
    ...
    stream.on("data", function (data: any) {
      ...
      if (testData !== undefined) {
        const promise = verifyAccountPostConditions(
          state,
          address,
          account,
          testData,
...
export function verifyGas(results: any, testData: any, t: tape.Test) {
  ...
  const preBal =
    testData.pre[coinbaseAddr] !== undefined
      ? testData.pre[coinbaseAddr].balance
      : 0;
...
export function makeBlockHeader(data: any, opts?: BlockOptions) {
  ...
  const parentBlockHeader = makeParentBlockHeader(data, {
    common: opts?.common,
  });
...
export function getDAOCommon(activationBlock: number) {
  ...
  const defaultDAOCommon = new Common({
    chain: Mainnet,
