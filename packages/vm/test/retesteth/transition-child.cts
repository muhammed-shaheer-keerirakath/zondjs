...
async function runTransition(argsIn: any) {
  const args = yargs(argsIn)
    ...
    .option("inputs.txs", {
      describe:
        "RLP input of txs to run on top of the initial state allocation",
    })
    .option("inputs.env", {
    ...
    .option("output.result", {
      describe:
        "File to write output results to (relative to `output.basedir`)",
    })
    .option("output.alloc", {
      describe:
        "File to write output allocation to (after running the transactions)",
    }).argv;
  ...
  const vm = blockchain
    ? await VM.create({ common, blockchain })
  ...
  const acc =
    (await vm.stateManager.getAccount(block.header.coinbase)) ?? new Account();
