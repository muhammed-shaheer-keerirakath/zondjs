...
import { Address, bytesToBigInt, hexToBytes } from "@theqrl/zondjs-util";
...
const testCases = [
  { chain: Mainnet, hardfork: Hardfork.Istanbul, selfbalance: "0xf1" },
  {
    ...
    err: EVMErrorMessage.INVALID_OPCODE,
  },
];
...
describe("Istanbul: EIP-1884", () => {
  it("SELFBALANCE", async () => {
    ...
    for (const testCase of testCases) {
      ...
      const balance =
        testCase.selfbalance !== undefined
      ...
      try {
        ...
        if (testCase.err !== undefined) {
          ...
          assert.ok(
            BigInt(testCase.selfbalance!) === bytesToBigInt(res.returnValue),
          );
        }
      } catch (e: any) {
