...
describe("VM -> Default EVM / Custom EVM Opts", () => {
  it("Default EVM should have correct default EVM opts", async () => {
    const vm = await createVM();
    assert.isFalse(
      (vm.evm as EVM).allowUnlimitedContractSize,
...
describe("VM -> setHardfork, blockchain", () => {
  ...
  describe("Ensure that precompile activation creates non-empty accounts", () => {
    it("should work", async () => {
      ...
      const code =
        "0x6000808080347300000000000000000000000000000000000000045AF100";
      /*
      ...
      */
      ...
      await vmNotActivated.stateManager.putCode(
        contractAddress,
        hexToBytes(code),
      ); // setup the contract code
      await vmNotActivated.stateManager.putAccount(
        caller,
        new Account(BigInt(0), BigInt(0x111)),
      ); // give calling account a positive balance
      await vmActivated.stateManager.putCode(contractAddress, hexToBytes(code)); // setup the contract code
      await vmActivated.stateManager.putAccount(
        caller,
        new Account(BigInt(0), BigInt(0x111)),
      ); // give calling account a positive balance
      ...
      const diff =
        resultNotActivated.execResult.executionGasUsed -
        resultActivated.execResult.executionGasUsed;
