...
describe("runTx() -> successful API parameter usage", async () => {
  ...
  it("simple run (unmodified options)", async () => {
    ...
    await simpleRun(
      vm,
      "mainnet (PoW), london HF, default SM - should run without errors",
    );
    ...
    await simpleRun(
      vm,
      "goerli (PoA), london HF, default SM - should run without errors",
    );
  });
  ...
  it("test hardfork mismatch", async () => {
    ...
    assert.ok(
      true,
      "runTx should not fail with mismatching hardforks if validation skipped",
    );
  });
  ...
  it("custom block (block option), disabled block gas limit validation (skipBlockGasLimitValidation: true)", async () => {
    for (const txType of TRANSACTION_TYPES) {
      ...
      const coinbaseAccount = await vm.stateManager.getAccount(
        new Address(coinbase),
      );
...
describe("runTx() -> API parameter usage/data errors", () => {
  it("Typed Transaction with HF set to pre-Berlin", async () => {
    ...
    const tx = getTransaction(
      new Common({ chain: Mainnet, hardfork: Hardfork.Berlin }),
      1,
      true,
    );
  ...
  it("run with insufficient funds", async () => {
    ...
    await vm.stateManager.putAccount(
      address,
      createAccountWithDefaults(BigInt(0), maxCost),
    );
    const res = await runTx(vm, { tx });
  ...
  it("run with insufficient eip1559 funds", async () => {
    ...
    try {
      ...
      assert.ok(
        true,
        "successfully threw on insufficient balance for transaction",
      );
    }
  });
  ...
  it("account balance overflows (create)", async () => {
    for (const txType of TRANSACTION_TYPES) {
      ...
      const contractAddress = createAddressFromString(
        "0x61de9dc6f6cff1df2809480882cfd3c2364b28f7",
      );
      const to = createAccountWithDefaults(BigInt(0), MAX_INTEGER);
...
describe("runTx() -> consensus bugs", () => {
  it("validate out-of-gas does not give any refunds", async () => {
    ...
    const txData = {
      blockHash:
        "0x78046e1c8f7956f2b654033c02f348b1c1d111701ec83cd9b5bf690e92efdedd",
      blockNumber: 2772981,
      gasLimit: 489999,
    ...
    const addr = createAddressFromString(
      "0xd3563d8f19a85c95beab50901fd59ca4de69174c",
    );
...
it("runTx() -> skipBalance behavior", async () => {
  ...
  for (const balance of [undefined, BigInt(5)]) {
    if (balance !== undefined) {
      await vm.stateManager.modifyAccountFields(sender, {
        nonce: BigInt(0),
        balance,
      });
    }
...
describe("EIP 4844 transaction tests", () => {
  ...
  it("should work", async () => {
    const { hardfork4844Data } = await import(
      "../../../block/test/testdata/4844-hardfork.js"
    );
    ...
    assert.ok(
      ...
      "simple blob tx run succeeds",
    );
    assert.equal(
      res.blobGasUsed,
      131072n,
      "returns correct blob gas used for 1 blob",
    );
