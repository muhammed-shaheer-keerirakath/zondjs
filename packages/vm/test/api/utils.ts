...
export function getTransaction(
  ...
  if (txType === TransactionType.AccessListEIP2930) {
    ...
    txParams["kzgCommitments"] = blobsToCommitments(
      common.customCrypto!.kzg!,
      txParams["blobs"],
    );
    txParams["kzgProofs"] = txParams["blobs"].map(
      (blob: PrefixedHexString, ctx: number) =>
        common.customCrypto!.kzg!.computeBlobProof(
          blob,
          txParams["kzgCommitments"][ctx],
        ),
    );
