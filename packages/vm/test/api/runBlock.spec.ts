...
describe("runBlock() -> successful API parameter usage", async () => {
  async function simpleRun(vm: VM) {
    ...
    const genesisRlp = hexToBytes(
      blockchainData.genesisRLP as PrefixedHexString,
    );
    const genesis = createBlockFromRLP(genesisRlp, { common });

    const blockRlp = hexToBytes(
      blockchainData.blocks[0].rlp as PrefixedHexString,
    );
...
describe("runBlock() -> runtime behavior", async () => {
  ...
  it("DAO fork behavior", async () => {
    ...
    const DAORefundAddress = new Address(
      hexToBytes("0xbf4ed7b27f1d666546e30d74d50d173d20bca754"),
    );
    const fundBalanceRefund = BigInt("0x4444");
    const accountRefund = createAccountWithDefaults(
      BigInt(0),
      fundBalanceRefund,
    );
    ...
    const DAOFundedContractAccount1 =
      (await vm.stateManager.getAccount(DAOFundedContractAddress1)) ??
      new Account();
    ...
    const DAOFundedContractAccount2 =
      (await vm.stateManager.getAccount(DAOFundedContractAddress2)) ??
      new Account();
  ...
  it("should allocate to correct clique beneficiary", async () => {
    ...
    await vm.stateManager.putAccount(
      otherUser.address,
      new Account(BigInt(0), BigInt(42000)),
    );
...
describe("runBlock() -> API return values", () => {
  it("should return correct HF receipts", async () => {
    ...
    assert.deepEqual(
      ...
      hexToBytes(
        "0x4477e2cfaf9fd2eed4f74426798b55d140f6a9612da33413c4745f57d7a97fcc",
      ),
...
describe("runBlock() -> tx types", async () => {
  async function simpleRun(vm: VM, transactions: TypedTransaction[]) {
    ...
    assert.equal(
      ...
      res.receipts
        ...
        .reduce(
          (prevValue: bigint, currValue: bigint) => prevValue + currValue,
          BigInt(0),
        ),
  ...
  it("eip7702 txs", async () => {
    ...
    function getAuthorizationListItem(
      ...
      const nonceBytes =
        nonce !== undefined
          ? unpadBytes(hexToBytes(`0x${nonce.toString(16)}`))
          : new Uint8Array();
      ...
      const yParity =
        signed.v === BigInt(27) ? new Uint8Array() : new Uint8Array([1]);
    ...
    const authList = authorizationListOpts.map((opt) =>
      getAuthorizationListItem(opt),
    );
    const authList2 = authorizationListOpts2.map((opt) =>
      getAuthorizationListItem(opt),
    );
...
describe.skip("run a verkle block", () => {
  it("should execute a verkle block and produce an executionWitness", async () => {
    const verkleJSONWithoutValue = (await import("./testdata/verkleBlock.js"))
      .block;
    const verkleJSONWithValue = (
      await import("./testdata/verkleBlockWithValue.js")
    ).block;
    ...
    for (const verkleJSON of verkleBlocks) {
      ...
      const blockRlp = hexToBytes(
        verkleJSON.blocks[0].rlp as PrefixedHexString,
      );
      const block = createBlockFromRLP(blockRlp, { common });
      ...
      vm.stateManager.initVerkleExecutionWitness?.(
        block.header.number,
        witness,
      );
      assert.equal(
