...
describe("EIP 2935: historical block hashes", () => {
  for (const deploymentConfig of deploymentConfigs) {
    ...
    async function testBlockhashContract(
      vm: VM,
      block: Block,
      i: bigint,
    ): Promise<Uint8Array> {
    ...
    it(`should validate the deployment tx`, async () => {
      ...
      const txSender = createAddressFromPublicKey(
        deployTx.getSenderPublicKey(),
      ).toString();
      assert.equal(
        toChecksumAddress(txSender),
        deploymentSender,
        "tx sender should match",
      );
      ...
      const txToAddress = bytesToHex(
        generateAddress(hexToBytes(txSender), bigIntToBytes(BIGINT_0)),
      );
      assert.equal(
    ...
    it("should ensure blocks older than 256 blocks can be retrieved from the history contract", async () => {
      ...
      const commonGetHistoryServeWindow = eip2935ActiveAtCommon(
        0,
        historyAddressBigInt,
      );
      commonGetHistoryServeWindow.setEIPs([2935]);
      commonGetHistoryServeWindow.updateParams(paramsVM);
      const common = eip2935ActiveAtCommon(
        blocksActivation,
        historyAddressBigInt,
      );
      const historyServeWindow =
        commonGetHistoryServeWindow.param("historyServeWindow");
      ...
      for (let i = 1; i <= blocksToBuild; i++) {
        ...
        const ret = await vm.evm.runCall({
          ...
          data: hexToBytes(
            "0x61" + i.toString(16).padStart(4, "0") + "4060205260406000F3",
          ),
          block: lastBlock,
        });
        ...
        if (
          ...
          if (i >= blocksToBuild - 256) {
            assert.ok(
              equalsBytes(
                ret.execResult.returnValue,
                setLengthLeft(block.hash(), 64),
              ),
            );
          } else {
            assert.ok(
              equalsBytes(ret.execResult.returnValue, new Uint8Array(64)),
            );
          }
        } else {
          assert.ok(
      ...
      for (const i of [blocksToBuild, blocksToBuild + 100]) {
        const blockHashI = await testBlockhashContract(vm, block, BigInt(i));
        assert.ok(
          equalsBytes(blockHashI, setLengthLeft(bigIntToBytes(BigInt(0)), 32)),
        );
      }
    }, 30_000);
  }
