...
async function runTest(
  ...
  const acc =
    (await vm.stateManager.getAccount(defaultSenderAddr)) ?? new Account();
...
describe("test EIP-7702 opcodes", () => {
  it("should correctly report EXTCODESIZE/EXTCODEHASH/EXTCODECOPY opcodes", async () => {
    ...
    async function runOpcodeTest(
      ...
      const result = await vm.stateManager.getStorage(
        deploymentAddress,
        new Uint8Array(32),
      );
    ...
    for (const test of tests) {
      await runOpcodeTest(
        hexToBytes(test.code),
        test.expectedStorage,
        test.name,
      );
    }
  });
