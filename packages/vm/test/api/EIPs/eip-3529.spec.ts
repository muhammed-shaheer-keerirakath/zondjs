...
describe("EIP-3529 tests", () => {
  ...
  it("refunds are capped at 1/5 of the tx gas used", async () => {
    ...
    assert.ok(
      result.gasRefund! > maxRefund,
      "refund is larger than the max refund",
    );
    assert.ok(
      result.totalGasSpent >= minGasUsed,
      "gas used respects the max refund quotient",
    );
  });
});
