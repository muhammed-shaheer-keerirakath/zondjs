...
describe("EIP4895 tests", () => {
  it("EIP4895: withdrawals execute as expected", async () => {
    ...
    */
    const withdrawalCheckAddress = new Address(
      hexToBytes(`0x${"fe".repeat(20)}`),
    );
    ...
    */
    ...
    assert.deepEqual(
      new Uint8Array(32),
      result!,
      "withdrawals happen after transactions",
    );

    const slotValue = await vm.stateManager.getStorage(
      withdrawalCheckAddress,
      new Uint8Array(32),
    );
    assert.deepEqual(
      new Uint8Array(),
      slotValue,
      "withdrawals do not invoke code",
    );
  });
  ...
  it("EIP4895: state update should exclude 0 amount updates", async () => {
    ...
    await vm.stateManager.generateCanonicalGenesis!(
      parseGethGenesisState(withdrawalsData),
    );
    ...
    const withdrawals = (gethBlockBufferArray[3] as WithdrawalBytes[]).map(
      (wa) => createWithdrawalFromBytesArray(wa),
    );
    assert.equal(
      withdrawals[0].amount,
      BigInt(0),
      "withdrawal 0 should have 0 amount",
    );
    let block: Block;
    ...
    block = createBlock(
      {
        header: {
          baseFeePerGas: BigInt(7),
          withdrawalsRoot: await genWithdrawalsTrieRoot(
            withdrawals.slice(0, 1),
          ),
  ...
  it("should build a block correctly with withdrawals", async () => {
    const common = createCommonFromGethGenesis(withdrawalsData, {
      chain: "custom",
    });
