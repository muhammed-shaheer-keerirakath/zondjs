...
describe("insert", () => {
  ...
  it("should set and check root", async () => {
    ...
    const invalidRootExists = await tree2.checkRoot(
    ...
    try {
      ...
      assert.equal(
        e.message,
        `Invalid root length. Roots are ${tree["_hashLen"]} bytes`,
      );
    }
  ...
  it("should handle one stem with colocated values", async () => {
    ...
    for (let i = 0; i < suffixes.length; i++) {
      ...
      assert.exists(
        retrievedValue,
        `Value at suffix ${suffixes[i]} should exist`,
      );
      assert.isTrue(
  ...
  it("should handle two stems with colocated values", async () => {
    ...
    for (let i = 0; i < suffixes1.length; i++) {
      ...
      assert.exists(
        retrievedValue1,
        `Value at suffix ${suffixes1[i]} should exist`,
      );
      assert.exists(
        retrievedValue2,
        `Value at suffix ${suffixes2[i]} should exist`,
      );
      assert.isTrue(
  ...
  it("should dump leaf values and node hashes", async () => {
    ...
    const expectedValues = keyValuePairs
    ...
    const expectedKeys = keyValuePairs
      .map(({ hashedKey }) => bytesToHex(hashedKey))
      .sort();
