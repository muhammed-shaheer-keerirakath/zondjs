...
export function decodeAccessedState(
  ...
  const position =
    BigInt(treeIndex) * BigInt(VERKLE_NODE_WIDTH) + BigInt(chunkIndex);
  switch (position) {
    case BigInt(0):
    ...
    default:
      if (position < VERKLE_HEADER_STORAGE_OFFSET) {
        throw Error(
          `No attribute yet stored >=2 and <${VERKLE_HEADER_STORAGE_OFFSET}`,
        );
      }

      if (
        position >= VERKLE_HEADER_STORAGE_OFFSET &&
        position < VERKLE_CODE_OFFSET
      ) {
        const slot = position - BigInt(VERKLE_HEADER_STORAGE_OFFSET);
        return { type: VerkleAccessedStateType.Storage, slot };
      } else if (
        position >= VERKLE_CODE_OFFSET &&
        position < VERKLE_MAIN_STORAGE_OFFSET
      ) {
...
export class VerkleAccessWitness implements VerkleAccessWitnessInterface {
  ...
  constructor(opts: {
    ...
    this.stems =
      opts.stems ?? new Map<PrefixedHexString, StemAccessEvent & StemMeta>();
  ...
  readAccountCodeChunks(
    ...
    for (
      ...
      const { treeIndex, subIndex } =
        getVerkleTreeIndicesForCodeChunk(chunkNum);
  ...
  readAccountStorage(address: Address, storageSlot: bigint): bigint {
    const { treeIndex, subIndex } =
      getVerkleTreeIndicesForStorageSlot(storageSlot);
  ...
  touchAddress(
    ...
    const accessedStemKey = getVerkleStem(
      this.verkleCrypto,
      address,
      treeIndex,
    );
