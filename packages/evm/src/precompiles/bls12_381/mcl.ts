...
function BLS12_381_ToG1Point(
  ...
  const p_y = bytesToUnprefixedHex(
    input.subarray(80, BLS_G1_POINT_BYTE_LENGTH),
  );
...
export class MCLBLS implements EVMBLSInterface {
  ...
  mulG1(input: Uint8Array): Uint8Array {
    // convert input to G1 points, add them, and convert the output to a Uint8Array.
    const p = BLS12_381_ToG1Point(
      input.subarray(0, BLS_G1_POINT_BYTE_LENGTH),
      this._mcl,
    );
    const frPoint = BLS12_381_ToFrPoint(
      input.subarray(BLS_G1_POINT_BYTE_LENGTH, 160),
      this._mcl,
    );
  ...
  mulG2(input: Uint8Array): Uint8Array {
    ...
    const p = BLS12_381_ToG2Point(
      input.subarray(0, BLS_G2_POINT_BYTE_LENGTH),
      this._mcl,
    );
    const frPoint = BLS12_381_ToFrPoint(
      input.subarray(BLS_G2_POINT_BYTE_LENGTH, 288),
      this._mcl,
    );
