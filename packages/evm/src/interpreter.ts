...
export class Interpreter {
  ...
  async _runStepHook(dynamicFee: bigint, gasLeft: bigint): Promise<void> {
    ...
    const eventObj: InterpreterStep = {
      ...
      memory: this._runState.memory._store.subarray(
        0,
        Number(this._runState.memoryWordCount) * 32,
      ),
  ...
  useGas(amount: bigint, context?: string | Opcode): void {
    ...
    if (this._evm.DEBUG) {
      ...
      debugGas(
        `${tempString}used ${amount} gas (-> ${this._runState.gasLeft})`,
      );
    }
  ...
  async storageLoad(key: Uint8Array, original = false): Promise<Uint8Array> {
    if (original) {
      return this._stateManager.originalStorageCache.get(
        this._env.address,
        key,
      );
    } else {
