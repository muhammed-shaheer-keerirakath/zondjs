...
describe("EIP 3860 tests", () => {
  it("code exceeds max initcode size", async () => {
    ...
    assert.ok(
      (result.execResult.exceptionError?.error as string) ===
  ...
  it("ensure EIP-3860 gas is applied on CREATE calls", async () => {
    ...
    const contractFactory = createAddressFromString(
    ...
    await evmWithout3860.stateManager.putAccount(
      contractFactory,
      contractAccount!,
    );
  ...
  it("code exceeds max initcode size: allowUnlimitedInitCodeSize active", async () => {
    ...
    const runCallArgs = {
      ...
      data: concatBytes(
        hexToBytes(
          `0x${"00".repeat(Number(common.param("maxInitCodeSize")) + 1)}`,
        ),
        bytes,
      ),
    };
  ...
  it("CREATE with MAX_INITCODE_SIZE+1, allowUnlimitedContractSize active", async () => {
    ...
    for (const code of ["F0", "F5"]) {
      ...
      const contractAccount =
        await evm.stateManager.getAccount(contractFactory);
      await evm.stateManager.putAccount(contractFactory, contractAccount!);
      await evmDisabled.stateManager.putAccount(
        contractFactory,
        contractAccount!,
      );
      // This factory code:
      ...
      const storageActive = await evm.stateManager.getStorage(
        contractFactory,
        key0,
      );
      const storageInactive = await evmDisabled.stateManager.getStorage(
        contractFactory,
        key0,
      );
