...
describe("verkle tests", () => {
  it("should execute bytecode and update the state", async () => {
    ...
    const retrievedValue = await sm.getStorage(
      address,
  ...
  it("should revert and access witness should not contain a write access due to OOG", async () => {
    ...
    const writtenChunks = Array.from(
      evm.verkleAccessWitness.chunks.entries(),
    ).filter(([_, chunk]) => chunk.write !== undefined);
...
describe("generate an execution witness", () => {
  it("should generate the correct execution witness from a prestate and changes", async () => {
    ...
    const stem = bytesToHex(
      getVerkleStem(verkle, createAddressFromString(tx.sender)),
    );
    assert.ok(
      executionWitness.stateDiff.findIndex((diff) => diff.stem === stem) !== -1,
    );
    const stemDiff =
      executionWitness.stateDiff[
        executionWitness.stateDiff.findIndex((diff) => diff.stem === stem)
      ];
    ...
    assert.equal(
      decodeVerkleLeafBasicData(hexToBytes(suffixDiff!.newValue!)).nonce,
      1n,
    );
  });
});
