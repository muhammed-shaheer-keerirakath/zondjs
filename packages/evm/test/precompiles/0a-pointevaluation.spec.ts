...
describe("Precompiles: point evaluation", () => {
  it("should work", async () => {
    const { eip4844Data } = await import(
      "../../../client/test/testdata/geth-genesis/eip4844.js"
    );
    ...
    const versionedHash = computeVersionedHash(
      testCase.commitment as PrefixedHexString,
      1,
    );
    ...
    const optsWithInvalidCommitment: PrecompileInput = {
      data: concatBytes(
        concatBytes(
          Uint8Array.from([0]),
          hexToBytes(versionedHash as PrefixedHexString),
        ),
        hexToBytes(testCase.z),
        hexToBytes(testCase.y),
        hexToBytes(testCase.commitment),
    ...
    assert.ok(
      res.exceptionError?.error.match("invalid input length"),
