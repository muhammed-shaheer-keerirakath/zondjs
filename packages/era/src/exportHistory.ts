...
export async function getHeader(DB: BlockDB, hash: Uint8Array, number: bigint) {
  const header = await dbGet(DB, DBTarget.Header, {
    blockHash: hash,
    blockNumber: number,
  });
...
export async function getTotalDifficulty(
  ...
  const td = await dbGet(DB, DBTarget.TotalDifficulty, {
    blockHash: hash,
    blockNumber: number,
  });
...
export async function getBlock(DB: BlockDB, number: bigint) {
  ...
  let body: BlockBodyBytes | undefined | Uint8Array = await getBody(
    DB,
    hash,
    number,
  );
...
export async function getBlockTuple(
  chainDB: BlockDB,
  metaDB: BlockDB,
  number: bigint,
) {
...
export async function getBlocks(
  ...
  for (let i = 0; i < number; i++) {
    const { blockHash, header, body, totalDifficulty, receipts } =
      await getBlockTuple(chainDB, metaDB, start + BigInt(i));
