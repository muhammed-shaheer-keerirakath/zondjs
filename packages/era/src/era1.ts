...
export const formatEra1 = async (
  ...
  const eraLength =
    version.length + blocksLength + accumulatorEntry.length + blockIndexLength;
  ...
  const offsets: Uint8Array[] = offsetBigInt.map((o) =>
    bigInt64ToBytes(o, true),
  );
...
export function readBlockIndex(data: Uint8Array, count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const offset = Number(
      new DataView(slotEntry.slice(0, 8).buffer).getBigInt64(0, true),
    );
    offsets.push(offset);
  }
  return {
...
export async function readOtherEntries(bytes: Uint8Array) {
  ...
  const lastTuple = readBlockTupleAtOffset(
    bytes,
    recordStart,
    offsets[count - 1],
  );
