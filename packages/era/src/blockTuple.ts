...
export async function createBlockTuples(
  blocks: Block[],
...
export function readBlockTupleAtOffset(
  bytes: Uint8Array,
  recordStart: number,
  offset: number,
) {
  ...
  const receiptsEntry = readEntry(
    bytes.slice(recordStart + offset + headerLength + bodyLength),
  );
  ...
  const totalLength =
    headerLength + bodyLength + receiptsLength + totalDifficultyLength;
  return {
    headerEntry,
    bodyEntry,
    receiptsEntry,
...
export function blockFromTuple({ header, body }: { header: any; body: any }) {
  const valuesArray = [
    ...
    body.data.txs,
    body.data.uncles,
    body.data.withdrawals,
  ];
  const block = createBlockFromBytesArray(valuesArray as BlockBytes, {
    setHardfork: true,
  });
  return block;
}
