...
describe("Read Era1", async () => {
  ...
  const { startingNumber, offsets } = readBlockIndex(
    blockIndex.data,
    blockIndex.count,
  );
  ...
  it("should read first block tuple", async () => {
    const tupleEntry = readBlockTupleAtOffset(
      era1File,
      blockIndex.recordStart,
      offsets[0],
    );
    const { header, totalDifficulty } = await parseBlockTuple(tupleEntry);
    const blockHeader = createBlockHeaderFromBytesArray(header.data, {
      setHardfork: true,
    });
