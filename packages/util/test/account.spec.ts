...
describe("Account", () => {
  ...
  it("from Array data", () => {
    ...
    const account = createAccountFromBytesArray(
      raw.map((el) => hexToBytes(el)),
    );
  ...
  it("serialize", () => {
    ...
    assert.ok(
      equalsBytes(account.serialize(), accountRlp),
  ...
  it("isContract", () => {
    ...
    assert.notOk(
      account.isContract(),
      "should return false for a non-contract account",
    );
    ...
    assert.ok(
      account.isContract(),
      "should return true for a contract account",
    );
  });
  ...
  it("isEmpty", () => {
    ...
    assert.notOk(
      account.isEmpty(),
      "should return false for a non-empty account",
    );
  });
...
describe("Utility Functions", () => {
  it("isValidPrivate", () => {
    ...
    assert.notOk(
      isValidPrivate(hexToBytes("0x0011223344")),
  ...
  it("isValidPublic", () => {
    ...
    assert.notOk(
      ...
      "should fail wt.testh an invalid SEC1 public key",
    ); // cspell:disable-line

    pubKey = hexToBytes(
      "0x03fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f",
    );
    assert.notOk(
      isValidPublic(pubKey),
      "should fail an invalid 33-byte public key",
    );
    ...
    assert.notOk(
      isValidPublic(pubKey),
      "should fail an invalid 64-byte public key",
    );
    ...
    assert.notOk(
      isValidPublic(pubKey, true),
      "should fail an invalid 65-byte public key",
    );

    pubKey = hexToBytes(
      "0x033a443d8381a6798a70c6ff9304bdc8cb0163c23211d11628fae52ef9e0dca11a",
    );
    assert.ok(
      isValidPublic(pubKey, true),
    ...
    assert.ok(
      isValidPublic(pubKey, true),
      "should work wt.testh sant.testize enabled",
    ); // cspell:disable-line
  ...
  it("privateToPublic", () => {
    ...
    privateKey =
      "0xea54bdc52d163f88c93ab0615782cf718a2efb9e51a7989aab1b08067e9c1c5f" as any;
    try {
  ...
  describe("toChecksumAddress()", () => {
    it("EIP55", () => {
      for (let i = 0; i < eip55ChecksumAddresses.length; i++) {
        ...
        assert.equal(
          toChecksumAddress(tmp.toLowerCase()).toLowerCase(),
          tmp.toLowerCase(),
        );
      }
    });
    ...
    describe("EIP1191", () => {
      it("Should encode the example addresses correctly", () => {
        for (const [chainId, addresses] of Object.entries(
          ...
          for (const addr of addresses) {
            assert.equal(
              toChecksumAddress(addr.toLowerCase(), Number(chainId)),
              addr,
            );
            assert.equal(
              toChecksumAddress(
      ...
      it("Should encode large chain ids greater than MAX_INTEGER correctly", () => {
        ...
        assert.equal(
          toChecksumAddress(addr.toLowerCase(), bytesToBigInt(chainIDBytes)),
          addr,
        );
        assert.equal(
          toChecksumAddress(addr.toLowerCase(), bytesToHex(chainIDBytes)),
          addr,
        );
      });
    });
  ...
  describe("isValidChecksumAddress()", () => {
    it("EIP55", () => {
      ...
      assert.notOk(
        isValidChecksumAddress("0x2f015c60e0be116b1f0cd534704db9c92118fb6a"),
      );
    });
  ...
  describe("isValidAddress()", () => {
    ...
    it("should return false", () => {
      ...
      assert.notOk(
        isValidAddress("0x2f015c60e0be116b1f0cd534704db9c92118fb6aa"),
      );
...
describe("createPartialAccountFromRLP", () => {
  ...
  const testCases = [
    ...
    {
      description:
        "should return a mix of null and non-null fields based on isNotNullIndicator",
      data: [
        [toBytes(1), toBytes(2)], // Nonce: 2
        [toBytes(0)], // Balance: null
        [toBytes(1), KECCAK256_RLP], // StorageRoot: KECCAK256_RLP
    ...
    {
      description:
        "should handle fields with empty arrays (isNullIndicator=0) correctly",
      data: [
...
describe("serializeWithPartialInfo", () => {
  const testCases = [
    ...
    {
      description:
        "should serialize all fields as non-null (isNotNullIndicator=1)",
      account: new Account(
        BigInt(2),
        Units.ether(1),
        KECCAK256_RLP,
    ...
    {
      description:
        "should correctly serialize when only some fields are provided",
