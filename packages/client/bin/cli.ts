...
async function startExecutionFrom(client: EthereumClient) {
  ...
  const startExecutionParent = await client.chain.getBlock(
    startExecutionBlock.header.parentHash,
  );
  ...
  if (
    client.config.execCommon.hardforkGteHardfork(
    ...
    if (client.config.statelessVerkle) {
      ...
      try {
        await client.chain.blockchain.setIteratorHead(
          "vm",
...
async function startClient(
  ...
  let blockchain;
  if (
    genesisMeta.genesisState !== undefined ||
    genesisMeta.genesisStateRoot !== undefined
  ) {
    ...
    if (config.statefulVerkle) {
      ...
      stateRoot = await generateVKTStateRoot(
        genesisMeta.genesisState,
        config.chainCommon,
      );
    }
...
const stopClient = async (
  ...
  if (clientStartPromise?.toString().includes("Promise") === true)
    ...
    timeoutHandle = setTimeout(() => {
      ...
      config.logger.warn(
        "Check logging output for potential errors.  Exiting...",
      );
      process.exit(1);
    }, 30000);
