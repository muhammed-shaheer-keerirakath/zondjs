  ...
  return (
    yargs
      ...
      .option("verkleGenesisStateRoot", {
        ...
        string: true,
        coerce: (customGenesisStateRoot: PrefixedHexString) =>
          hexToBytes(customGenesisStateRoot),
      })
      .option("gethGenesis", {
      ...
      .option("statefulVerkle", {
        describe:
          "Run verkle+ hardforks using stateful verkle stateManager (experimental)",
        boolean: true,
        default: false,
      })
...
export async function generateClientConfig(args: ClientOpts) {
  ...
  const config = new Config({
    ...
    statelessVerkle:
      args.ignoreStatelessInvalidExecs === true ? true : args.statelessVerkle,
    statefulVerkle: args.statefulVerkle,
    startExecution:
      args.startExecutionFrom !== undefined ? true : args.startExecution,
