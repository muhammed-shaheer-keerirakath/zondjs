...
export class VMExecution extends Execution {
  ...
  async setupVerkleVM() {
    ...
    if (this.config.statelessVerkle) {
      this.config.logger.info(
        `Setting up verkleVM for stateless verkle execution`,
      );
      ...
      this.config.logger.info(
        `Setting up verkleVM for stateful verkle execution`,
      );
      stateManager = new StatefulVerkleStateManager({
        common: this.config.execCommon,
      });
    } else
      throw new Error("EIP-6800 active and no verkle execution mode specified");
    await mcl.init(mcl.BLS12_381);
    const rustBN = await initRustBN();
