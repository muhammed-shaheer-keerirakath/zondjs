    ...
    if (this.config.syncmode === SyncMode.Full) {
      if (this.config.chainCommon.gteHardfork(Hardfork.Paris)) {
        ...
        this.config.logger.info(
          `Post-merge 🐼 client mode: run with CL client.`,
        );
      } else {
      ...
      for (const addr of this.txPool.pool) {
        for (const tx of addr[1]) {
          ...
          if (rawTx.type !== TransactionType.BlobEIP4844) {
            ...
            txs[1].push(
              (rawTx as Blob4844Tx).serializeNetworkWrapper().byteLength,
            );
          }
    ...
    try {
      if (this.execution.started && this.synchronizer !== undefined) {
        ...
        if (
          ...
          this.skeleton?.synchronized === true
        ) {
          ...
          if (syncResult !== null) {
            ...
            if (transition === true) {
              this.config.superMsg(
                "Snapsync completed, transitioning to VMExecution",
              );
              await this.execution.open();
              await this.execution.start();
            }
