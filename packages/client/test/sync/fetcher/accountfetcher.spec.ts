...
import { bytesToBigInt, hexToBytes } from "@theqrl/zondjs-util";
...
describe("[AccountFetcher]", async () => {
  ...
  const { AccountFetcher } = await import(
    "../../../src/sync/fetcher/accountfetcher.js"
  );
  ...
  it("should verify proof correctly", async () => {
    ...
    assert.ok(
      fetcher.storageFetcher !== undefined,
      "storageFetcher should be created",
    );
    ...
    const snapCompleted = new Promise((resolve) => {
      config.events.once(Event.SYNC_SNAPSYNC_COMPLETE, (stateRoot: any) =>
        resolve(stateRoot),
      );
    });
