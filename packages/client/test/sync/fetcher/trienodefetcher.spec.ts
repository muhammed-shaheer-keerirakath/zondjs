import { decodeMPTNode } from "@theqrl/zondjs-mpt";
...
import type { BranchMPTNode } from "@theqrl/zondjs-mpt";
...
describe("[TrieNodeFetcher]", async () => {
  ...
  const { TrieNodeFetcher } = await import(
    "../../../src/sync/fetcher/trienodefetcher.js"
  );
  ...
  it("should process", async () => {
    ...
    assert.notOk(
      fetcher.process({} as any, { NodeDataResponse: [] } as any),
      "bad results",
    );
  });
  ...
  it("should generate child paths for node correctly", async () => {
    ...
    const rootNode = decodeMPTNode(
      nodes[0] as unknown as Uint8Array,
    ) as BranchMPTNode;
