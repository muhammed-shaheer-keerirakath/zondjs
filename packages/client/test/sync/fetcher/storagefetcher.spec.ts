...
describe("[StorageFetcher]", async () => {
  ...
  const { StorageFetcher } = await import(
    "../../../src/sync/fetcher/storagefetcher.js"
  );
  ...
  it("should start/stop", async () => {
    ...
    assert.equal(
      (fetcher as any).storageRequests.length,
      1,
      "one storageRequests have been added",
    );
    ...
    assert.equal(
      job!.task.storageRequests.length,
      2,
  ...
  it("should process", () => {
    ...
    assert.throws(() =>
      fetcher.process({} as any, { StorageDataResponse: [] } as any),
    );
  });
  ...
  it("should request correctly", async () => {
    ...
    assert.notOk(
      ret,
      "Should stop requesting from peer that rejected storage request",
    );
  });
  ...
  it("should verify proof correctly", async () => {
    ...
    assert.ok(
      results !== undefined,
      "Proof verification is completed without errors",
    );
    ...
    assert.equal(
      results![0].length,
      3,
      "3 results should be there with dummy partials",
    );
    ...
    assert.ok(
      fetcher["destroyWhenDone"] === true,
      "should mark to close on finished",
    );
  });
