...
const { BlockFetcher } = await import(
  "../../../src/sync/fetcher/blockfetcher.js"
);
...
describe("[BlockFetcher]", async () => {
  ...
  it("enqueueByNumberList()", async () => {
    ...
    assert.equal(
      ...
      3,
      "1 new task for two subsequent block numbers",
    );
    ...
    assert.equal(
      (fetcher as any).in.length,
      3,
      "no new task added only the height changed",
    );
    assert.equal(
  ...
  it("should parse bodies correctly", async () => {
    ...
    assert.equal(
      resp[0].withdrawals?.length,
      0,
      "should have withdrawals array",
    );
  });
});
describe("store()", async () => {
  ...
  it("should error", async () => {
    chain.putBlocks = vi
      .fn()
      .mockRejectedValueOnce(new Error("could not find parent header"));
    try {
      await fetcher.store([]);
