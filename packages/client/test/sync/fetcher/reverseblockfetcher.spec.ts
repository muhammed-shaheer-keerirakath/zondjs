...
const { ReverseBlockFetcher } = await import(
  "../../../src/sync/fetcher/reverseblockfetcher.js"
);
describe("[ReverseBlockFetcher]", async () => {
  it("should start/stop", async () => {
    ...
    assert.equal(
      fetcher.first,
      BigInt(14),
      "pending tasks first tracking should be reduced",
    );
    assert.equal(
      fetcher.count,
      BigInt(0),
      "pending tasks count should be reduced",
    );
    fetcher.destroy();
    await wait(100);
  ...
  it("should generate max tasks", async () => {
    ...
    assert.equal(
      fetcher.count,
      BigInt(3),
      "pending tasks count should be reduced by maximum",
    );
    fetcher.destroy();
    await wait(100);
    assert.notOk((fetcher as any).running, "stopped");
  ...
  it("should process", async () => {
    ...
    assert.deepEqual(
      fetcher.process({ task: { count: 2 } } as any, blocks),
      blocks,
      "got results",
    );
    assert.notOk(
  ...
  it("should restart the fetcher when subchains are merged", async () => {
    ...
    assert.equal(
      (fetcher as any).out.length,
      0,
      "fetcher out should be cleared",
    );
  });
});
describe("store()", async () => {
  ...
  it("should error", async () => {
    ...
    try {
      ...
      assert.equal(
        destroyFetcher,
        false,
        "fetcher should not be destroyed on this error",
      );
  ...
  config.events.on(Event.SYNC_FETCHED_BLOCKS, () =>
    it("should emit event on put blocks", async () => {
      assert.ok(
        true,
        "store() emitted SYNC_FETCHED_BLOCKS event on putting blocks",
      );
    }),
  );
  await fetcher.store([]);
