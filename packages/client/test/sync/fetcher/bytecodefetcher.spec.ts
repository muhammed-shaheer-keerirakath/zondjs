...
describe("[ByteCodeFetcher]", async () => {
  ...
  const { ByteCodeFetcher } = await import(
    "../../../src/sync/fetcher/bytecodefetcher.js"
  );
  ...
  it("should start/stop", async () => {
    ...
    assert.equal(
      (fetcher as any).hashes.length,
      1,
      "one codehash have been added",
    );
    ...
    assert.equal(
      job!.task.hashes.length,
      2,
      "two storageRequests are added to job",
    );
    ...
    assert.ok(
      fetcher.write() === false,
      "fetcher should not setup a new write pipe",
    );
    fetcher.destroy();
    await wait(100);
  ...
  it("should process", () => {
    ...
    assert.notOk(
      fetcher.process({} as any, { ByteCodeResponse: [] } as any),
      "bad results",
    );
  });
  ...
  it("should adopt correctly", () => {
    ...
    assert.equal(
      ...
      1,
      "Fetcher should still have same job",
    );
    assert.equal(
      job?.partialResult.length,
      2,
      "Should have two partial results",
    );
    assert.equal(
      results,
      undefined,
      "Process should not return full results yet",
    );
    const remainingBytesCodeData: any = [
      utf8ToBytes(""),
      utf8ToBytes(""),
      utf8ToBytes(""),
    ];
  ...
  it("should request correctly", async () => {
    ...
    const task = {
      hashes: [
        hexToBytes(
          "0x28ec5c6e71bc4243030bc6aa069616b4497c150c883c019dee059279f0593cd8",
        ),
        hexToBytes(
          "0x418df730969850c4f5c10d09ca929d018ee4c5d71243aa7440560e2265c37aab",
        ),
        hexToBytes(
          "0x01b45b4d94f26e3f7a84ea31f7338c0f621d3f3ee38e439611a0954da7e2d728",
        ),
        hexToBytes(
          "0x6bd103c66d7d0908a75ae23d5f6de62865be2784408cf07906eaffe515616212",
        ),
        hexToBytes(
          "0x0c9d7b40fa7bb308c9b029f7b2840bc1071760c55cdf136b08f0f81ace379399",
        ),
      ],
    };
    ...
    assert.ok(
      ...
      "response processed and matched properly",
    );
    ...
    try {
      ...
      assert.fail(
        `fetcher failed to store results, Error: ${(e as Error).message}`,
      );
    }
  });
