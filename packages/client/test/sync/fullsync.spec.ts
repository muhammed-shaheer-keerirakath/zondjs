...
describe("[FullSynchronizer]", async () => {
    ...
    const peer = {
      eth: {
        getBlockHeaders: vi.fn((input) => {
          ...
          if (
            JSON.stringify(input) === JSON.stringify({ block: "hash", max: 1 })
          )
  ...
  it("should send NewBlock/NewBlockHashes to right peers", async () => {
    ...
    assert.equal(
      config.syncTargetHeight,
      BigInt(0),
      "sync target height should be set to 0",
    );
  ...
  it("should process blocks", async () => {
    ...
    sync.running = true;
    assert.ok(
      await sync.processBlocks([newBlock]),
