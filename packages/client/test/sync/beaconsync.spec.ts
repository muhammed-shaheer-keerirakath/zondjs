...
describe("[BeaconSynchronizer]", async () => {
  ...
  it("should initialize correctly", async () => {
    ...
    const sync = new BeaconSynchronizer({
      config,
      pool,
      chain,
      execution,
  ...
  it("should get height", async () => {
    ...
    td.when(peer.eth.getBlockHeaders({ block: "hash", max: 1 })).thenResolve([
      BigInt(1),
      headers,
    ]);
  ...
  it("should find best", async () => {
    ...
    const peers = [
      {
        eth: {
          getBlockHeaders: td.func(),
          status: { bestHash: "hash1" },
          inbound: false,
        },
        latest: () => {
          return {
    ...
    td.when(
      peers[0].eth.getBlockHeaders({ block: "hash1", max: 1 }),
    ).thenResolve([BigInt(1), [{ number: BigInt(5) }]]);
    td.when(
      peers[1].eth.getBlockHeaders({ block: "hash2", max: 1 }),
    ).thenResolve([BigInt(1), [{ number: BigInt(10) }]]);
    (sync as any).pool = { peers };
  ...
  it("should sync to next subchain head or chain height", async () => {
    ...
    td.when(ReverseBlockFetcher.prototype.fetch(), {
      delay: 100,
      times: 3,
    }).thenResolve(false);
  ...
  it("should extend and set with a valid head", async () => {
    ...
    assert.ok(
      await sync.extendChain(block),
      "should extend chain successfully",
    );
    ...
    assert.notOk(
      await sync.extendChain(gapBlock),
      "should not extend chain with gapped block",
    );
    assert.ok(
    ...
    assert.equal(
      skeleton.bounds().head,
      BigInt(18),
      "head should update with gapped block",
    );
    await sync.stop();
    await sync.close();
  });
