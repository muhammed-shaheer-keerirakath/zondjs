...
describe("[Skeleton]/ startup scenarios ", () => {
  ...
  it("throws when reset called before being started", async () => {
    ...
    try {
      ...
      assert.ok(
        err.message.includes("skeleton reset"),
...
describe("[Skeleton] / setHead", async () => {
  ...
  it(`skeleton init should throw error if merge not set`, async () => {
    ...
    const common = createCommonFromGethGenesis(genesis, {
      chain: "merge-not-set",
    });
  ...
  it("should init/setHead properly from genesis", async () => {
    ...
    assert.equal(
      chain.blocks.height,
      BigInt(0),
      "canonical height should be at genesis",
    );
    ...
    assert.equal(
      skeleton.isLinked(),
      true,
      "subchain status should stay linked",
    );
    ...
    assert.equal(
      skeleton.isLinked(),
      false,
      "subchain status should not be linked anymore",
    );
  });
  ...
  it("should fill the canonical chain after being linked to a canonical block past genesis", async () => {
    ...
    assert.equal(
      chain.blocks.height,
      BigInt(2),
      "canonical height should be at block 2",
    );
