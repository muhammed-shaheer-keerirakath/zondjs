  ...
  const service: any = {
    ...
    execution: {
      vm: {
        stateManager: {
          getAccount: () =>
            new Account(BigInt(0), BigInt("50000000000000000000")),
...
const handleTxs = async (
  ...
  try {
    if (stateManager !== undefined) {
      ...
      pool["service"].execution.vm.stateManager.setStateRoot = async (
        _root: Uint8Array,
      ) => {};
    }
...
describe("[TxPool]", async () => {
  ...
  const createTx = (from = A, to = B, nonce = 0, value = 1, feeBump = 0) => {
    ...
    txData.maxPriorityFeePerGas +=
      (txData.maxPriorityFeePerGas * feeBump) / 100;
      ...
      eth: {
        ...
        send: () => {
          ...
          assert.equal(
            sentToPeer2,
            1,
            "should send once to non-announcing peer",
          );
        },
        request: () => {
          sentToPeer2++;
  ...
  it("announcedTxHashes() -> reject underpriced txn (same sender and nonce)", async () => {
    ...
    assert.equal(
      pool["knownByPeer"].get(peer2.id)?.length,
      1,
      "known send objects",
    );
    assert.equal(
  ...
  it("announcedTxHashes() -> reject if pool is full", async () => {
    ...
    assert.notOk(
      await handleTxs(txs, "pool is full"),
      "successfully rejected too many txs",
    );
  });
  ...
  it("announcedTxHashes() -> reject txs with invalid nonce", async () => {
    ...
    assert.notOk(
      await handleTxs(txs, "tx nonce too low", {
        getAccount: () =>
          new Account(BigInt(1), BigInt("50000000000000000000")),
      } as any),
  ...
  it("announcedTxHashes() -> reject txs with too much data", async () => {
    ...
    assert.notOk(
      await handleTxs(txs, "exceeds the max data size", {
        getAccount: () =>
          new Account(BigInt(0), BigInt("50000000000000000000000")),
      } as any),
  ...
  it("announcedTxHashes() -> reject txs which cannot pay base fee", async () => {
    ...
    assert.notOk(
      await handleTxs(
        txs,
        "not within 50% range of current basefee",
        undefined,
        pool,
      ),
  ...
  it("announcedTxHashes() -> reject txs with too low gas price (invalid tx type)", async () => {
    ...
    assert.notOk(
      await handleTxs(txs, ""),
      "successfully rejected tx with invalid tx type",
    );
  });
  ...
  it("cleanup()", async () => {
    ...
    knownByPeerObj1.added =
      Date.now() - pool.POOLED_STORAGE_TIME_LIMIT * 1000 * 60 - 1;
    ...
    handledObj.added =
      Date.now() - pool.HANDLED_CLEANUP_TIME_LIMIT * 1000 * 60 - 1;
