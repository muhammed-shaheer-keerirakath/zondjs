...
RlpxPeer.prototype.accept = vi.fn((input: object) => {
  if (!(input instanceof RlpxPeer))
    throw Error("expected RlpxPeer type as input");
});
  ...
  it("should initialize correctly", async () => {
    ...
    assert.deepEqual(
      ...
      [
        ...
        multiaddr("/ip4/10.0.0.2/tcp/1234"),
      ],
      "bootnodes split",
    );
  });
...
describe("should handles errors from id-less peers", async () => {
  ...
  RlpxPeer.prototype.accept = vi.fn((input) => {
    if (
      JSON.stringify(input[0]) === JSON.stringify(rlpxPeer) &&
      input[1] instanceof RlpxPeer
    ) {
      return;
    } else {
      throw new Error("expected input check has failed");
...
describe("should init rlpx", async () => {
  ...
  config.events.on(Event.SERVER_LISTENING, (info) =>
    it("should listen", async () => {
      assert.deepEqual(
        info,
        { transport: "rlpx", url: "enode://ff@0.0.0.0:30303" },
        "listening",
      );
    }),
  );
