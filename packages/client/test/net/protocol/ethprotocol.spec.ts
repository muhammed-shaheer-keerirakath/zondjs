...
describe("[EthProtocol]", () => {
  ...
  it("verify that NewBlock handler encodes/decodes correctly", async () => {
    ...
    const res = p.decode(
      p.messages.filter((message) => message.name === "NewBlock")[0],
      [block.raw(), bigIntToBytes(td)],
    );
    const res2 = p.encode(
      p.messages.filter((message) => message.name === "NewBlock")[0],
      [block, td],
  ...
  it("verify that GetReceipts handler encodes/decodes correctly", async () => {
    ...
    const res = p.decode(
      p.messages.filter((message) => message.name === "GetReceipts")[0],
      [bigIntToBytes(1n), [block.hash()]],
    );
    const res2 = p.encode(
      p.messages.filter((message) => message.name === "GetReceipts")[0],
      {
  ...
  it("verify that PooledTransactions handler encodes correctly", async () => {
    ...
    const res = p.encode(
      p.messages.filter((message) => message.name === "PooledTransactions")[0],
      {
        reqId: BigInt(1),
        txs: [tx],
      },
    ...
    assert.deepEqual(
      res[1][0],
      tx.serialize(),
      "EIP1559 transaction correctly encoded",
    );
  });
  ...
  it("verify that Receipts encode/decode correctly", async () => {
    ...
    // encode
    let res = p.encode(
      p.messages.filter((message) => message.name === "Receipts")[0],
      {
        reqId: BigInt(1),
        receipts,
      },
    ...
    assert.deepEqual(
      res[1],
      expectedSerializedReceipts,
      "correctly encoded receipts",
    );
    ...
    assert.deepEqual(
      res[1],
      receiptsWithoutTxType,
      "receipts correctly decoded",
    );
  });
  ...
  it("verify that Transactions handler encodes/decodes correctly", async () => {
    ...
    const res = p.encode(
      ...
      [legacyTx, eip2929Tx, eip1559Tx, blobTx],
    );
    assert.deepEqual(
      res[0],
      legacyTx.serialize(),
      "legacy tx correctly encoded",
    );
    assert.deepEqual(
      res[1],
      eip2929Tx.serialize(),
      "EIP29292 tx correctly encoded",
    );
    assert.deepEqual(
      res[2],
      eip1559Tx.serialize(),
      "EIP1559 tx correctly encoded",
    );
    ...
    assert.deepEqual(
      decoded[0].type,
      legacyTx.type,
      "decoded legacy tx correctly",
    );
    assert.deepEqual(
      decoded[1].type,
      eip2929Tx.type,
      "decoded eip2929 tx correctly",
    );
    assert.deepEqual(
      decoded[2].type,
      eip1559Tx.type,
      "decoded EIP1559 tx correctly",
    );
  ...
  it("verify that NewPooledTransactionHashes encodes/decodes correctly", async () => {
    ...
    const encoded = p.encode(
      p.messages.filter(
        (message) => message.name === "NewPooledTransactionHashes",
      )[0],
      [fakeHash],
    );
    ...
    assert.deepEqual(
      encoded[0],
      fakeHash,
      "encoded hash correctly with pre-eth/68 format",
    );
    assert.deepEqual(
      encodedEth68[2][0],
      fakeHash,
      "encoded hash correctly with eth/68 format",
    );
    ...
    assert.deepEqual(
      decoded[0],
      fakeHash,
      "decoded hash correctly with pre-eth/68 format",
    );
    assert.deepEqual(
      decodedEth68[2][0],
      fakeHash,
      "decoded hash correctly with eth/68 format",
    );
  });
});
