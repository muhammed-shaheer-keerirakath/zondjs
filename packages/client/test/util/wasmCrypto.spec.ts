...
describe("WASM crypto tests", () => {
  it("should compute public key and hash correctly using common.customCrypto functions", async () => {
    ...
    const tx2 = createLegacyTx({}, { common: commonWithCustomCrypto }).sign(
      pk,
      false,
    );
  ...
  it("should compute the same signature whether js or WASM signature used", async () => {
    ...
    assert.deepEqual(
      wasmSig,
      jsSig,
      "wasm signatures produce same result as js signatures",
    );
    assert.throws(
  ...
  it("should have the same signature and verification", async () => {
    await waitReady();
    const pk = hexToBytes(
      "0xbd3713a6da2c3624fa10bad8a52848b4291e3c9689ab50e0d2761e014d6e4cd7",
    );
    const hash = hexToBytes(
      "0x8c6d72155f746a9424b0621d82c5f5d3f6cc82e497b15df1b2ae601c8c14f75c",
    );
