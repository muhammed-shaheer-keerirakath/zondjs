...
async function createSnapClient(
  ...
  const ejsInlineClient = await createInlineClient(
    config,
    common,
    customGenesisState,
    datadir,
  );
  const beaconSyncRelayer = await setupEngineUpdateRelay(
    ejsInlineClient,
    peerBeaconUrl,
  );
...
describe("simple mainnet test run", async () => {
  ...
  it.skipIf(process.env.SNAP_SYNC === undefined)(
    ...
    async () => {
      ...
      assert.equal(
        res.result,
        true,
        "successfully requested Geth add EthereumJS as peer",
      );

      const peerConnectTimeout = new Promise((_resolve, reject) =>
  ...
  it.skipIf(process.env.SNAP_SYNC === undefined)(
    ...
    async () => {
      if (
        ...
        const snapSyncTimeout = new Promise((_resolve, reject) =>
          setTimeout(reject, 8 * 60_000),
        );
        ...
        try {
          ...
          await Promise.race([
            (snapCompleted as any).then(
              ([root, syncedStateManager]: [any, any]) => {
                syncedSnapRoot = root;
        ...
        const peerLatest = (
          await client.request("eth_getBlockByNumber", ["latest", false])
        ).result;
        const snapRootsMatch =
          syncedSnapRoot !== undefined &&
          bytesToHex(syncedSnapRoot) === peerLatest.stateRoot;
  ...
  it.skipIf(stateManager !== undefined)(
    "should match entire state",
    async () => {
      ...
      if (process.env.ADD_EOA_STATE !== undefined) {
        ...
        (customGenesisState[sender][0] as any) =
          `0x${senderBalance.toString(16)}`;
      }
