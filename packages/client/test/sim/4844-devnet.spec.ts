...
describe("sharding/eip4844 hardfork tests", async () => {
  if (process.env.EXTRA_CL_PARAMS === undefined) {
    process.env.EXTRA_CL_PARAMS =
      "--params.CAPELLA_FORK_EPOCH 0 --params.DENEB_FORK_EPOCH 0";
  }
  ...
  it(
    ...
    async () => {
      ...
      let txReceipt;
      while (!done) {
        txReceipt = await client.request(
      ...
      assert.ok(
        BigInt(block2.result.excessBlobGas) > 0n,
        "block1 has excess blob gas > 0",
      );
    },
    10 * 60_000,
  );
