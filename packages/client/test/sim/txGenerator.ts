...
async function run(data: any) {
  ...
  while (!done) {
    const num = parseInt(
      (await client.request("eth_blockNumber", [], 2.0)).result,
    );
    if (num >= 1) {
      done = true;
      break;
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      while (!done) {
        ...
        console.log(
          `waiting for tx to be included in block.... block number=${current}`,
        );
        await sleep(1000);
      }
    }
    ...
    console.log(error);
    console.log(
      `Error retrieving blocks from ${error.config.url}: ${error.response.data}`,
    );
    return false;
  }
  ...
  if (blob_kzg !== blobTx.kzgCommitments![0]) {
    console.log(
      `Unexpected KZG commitment: expected ${expected_kzgs}, got ${blob_kzg}`,
    );
    return false;
  } else {
