...
const privateKey =
  "dc6457099f127cf0bac78de8b297df04951281909db4f58b43def7c7151e765d";
const privateKeyBytes = hexToBytes("0x" + privateKey);

describe(method, () => {
  it("works", async () => {
    const localPeerClient = await createClient({ opened: true, noPeers: true });
    const remotePeerClient = await createClient({
      opened: true,
      noPeers: true,
    });
    const rpc = getRPCClient(
      startRPC(createManager(localPeerClient).getMethods()),
    );
    ...
    assert.equal(
      addPeerResponse.result,
      true,
      "admin_addPeer successfully adds peer",
    );
