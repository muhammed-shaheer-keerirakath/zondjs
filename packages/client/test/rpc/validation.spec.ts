...
describe(prefix, () => {
  ...
  it("address", () => {
    ...
    assert.ok(
      validatorResult(
        validators.address(["0x0000000000000000000000000000000000000000"], 0),
      ),
    );
    // lowercase address
    assert.ok(
      validatorResult(
        validators.address(["0xa7d8d9ef8d8ce8992df33d8b8cf4aebabd5bd270"], 0),
      ),
    );
    // checksummed address
    assert.ok(
    ...
    assert.notOk(
      validatorResult(
        validators.address(["0x00000000000000000000000000000000000000"], 0),
      ),
    );
    // invalidlength: 39 chars
    assert.notOk(
      validatorResult(
        validators.address(["0x000000000000000000000000000000000000000"], 0),
      ),
    );
    // invalidlength: 41 chars
    assert.notOk(
      validatorResult(
        validators.address(["0x00000000000000000000000000000000000000000"], 0),
      ),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.address(["0x62223651d6a33d58be70eb9876c3caf7096169ez"], 0),
      ),
    );
    assert.ok(
      validatorResult(validators.bytes8([bytesToHex(randomBytes(8))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes8([bytesToUnprefixedHex(randomBytes(8))], 0),
      ),
    );
  });
  ...
  it("Uint64", () => {
    // valid
    assert.ok(
      validatorResult(validators.uint64([bytesToHex(randomBytes(8))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.uint64([bytesToUnprefixedHex(randomBytes(8))], 0),
      ),
    );
  });
  it("Bytes16", () => {
    // valid
    assert.ok(
      validatorResult(validators.bytes16([bytesToHex(randomBytes(16))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes16([bytesToUnprefixedHex(randomBytes(16))], 0),
      ),
    );
  });
  it("Bytes20", () => {
    ...
    assert.ok(
      validatorResult(validators.bytes20([bytesToHex(randomBytes(20))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes20([bytesToUnprefixedHex(randomBytes(20))], 0),
      ),
    );
  });
  it("Bytes32", () => {
    // valid
    assert.ok(
      validatorResult(validators.bytes32([bytesToHex(randomBytes(32))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes32([bytesToUnprefixedHex(randomBytes(32))], 0),
      ),
    );
  });
  it("Uint256", () => {
    // valid
    assert.ok(
      validatorResult(validators.uint256([bytesToHex(randomBytes(32))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.uint256([bytesToUnprefixedHex(randomBytes(32))], 0),
      ),
    );
  });
  it("Bytes48", () => {
    // valid
    assert.ok(
      validatorResult(validators.bytes48([bytesToHex(randomBytes(48))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes48([bytesToUnprefixedHex(randomBytes(48))], 0),
      ),
    );
  });
  it("Bytes256", () => {
    // valid
    assert.ok(
      validatorResult(validators.bytes256([bytesToHex(randomBytes(256))], 0)),
    );
    ...
    assert.notOk(
      validatorResult(
        validators.bytes256([bytesToUnprefixedHex(randomBytes(256))], 0),
      ),
    );
  });
  ...
  it("unsignedInteger", () => {
    ...
    assert.ok(
      validatorResult(validators.unsignedInteger([Number.MAX_SAFE_INTEGER], 0)),
    );

    assert.notOk(validatorResult(validators.unsignedInteger([-1], 0)));
    assert.notOk(
      validatorResult(
        validators.unsignedInteger([Number.MAX_SAFE_INTEGER + 1], 0),
      ),
    );
    assert.notOk(
      validatorResult(validators.unsignedInteger([Number.MIN_VALUE], 0)),
    );
    assert.notOk(
      validatorResult(validators.unsignedInteger([Number.MAX_VALUE], 0)),
    );
    assert.notOk(
      validatorResult(
        validators.unsignedInteger([Number.NEGATIVE_INFINITY], 0),
      ),
    );
    assert.notOk(
      validatorResult(
        validators.unsignedInteger([Number.POSITIVE_INFINITY], 0),
      ),
    );
    assert.notOk(validatorResult(validators.unsignedInteger([Number.NaN], 0)));
    assert.notOk(
      validatorResult(validators.unsignedInteger([Number.EPSILON], 0)),
    );
    ...
    assert.notOk(
      validatorResult(validators.unsignedInteger([{ number: 1 }], 0)),
    );
  ...
  describe("byteVectors", () => {
    ...
    it("Bytes256", () => {
      ...
      assert.notOk(validatorResult(validators.bytes256([bytes(512)], 0)));
      assert.notOk(
        validatorResult(validators.bytes256([bytes(256, false)], 0)),
      );
    });
  });
  ...
  it("transaction", () => {
    ...
    assert.notOk(
      validatorResult(validators.transaction(["to"])([{ gas: "0xcf08" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction(["to"])([{ to: "0x" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction(["to"])([{ to: "0x0" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction(["to"])([{ to: "0x00" }], 0)),
    );
    assert.notOk(
      validatorResult(
        validators.transaction(["to"])(
    ...
    assert.notOk(
      validatorResult(validators.transaction([])([{ gas: "12" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction([])([{ gasPrice: "12" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction([])([{ value: "12" }], 0)),
    );
    assert.notOk(
      validatorResult(validators.transaction([])([{ data: "12" }], 0)),
    );
  });
  ...
  it("object", () => {
          ...
          [
            {
              ...
              blockHash:
                "0x4152dae052dceaeaeff588aec17a88679fc61aa0c0ca362a2572b94f9c542b24",
              bool: true,
              hex: "0x1",
            },
          ],
    ...
    assert.notOk(
      validatorResult(
        validators.object({ address: validators.address })(
          [{ address: "0x0" }],
          0,
        ),
      ),
    ...
    assert.notOk(
      validatorResult(
        validators.object({ blockHash: validators.blockHash })(
          [{ blockHash: "0x0" }],
          0,
        ),
      ),
    ...
    assert.notOk(
      validatorResult(
        validators.object({ bool: validators.bool })([{ bool: "0x0" }], 0),
      ),
    );
    assert.notOk(
      validatorResult(
        validators.object({ hex: validators.hex })([{ hex: "1" }], 0),
      ),
    );
  });
  ...
  it("array", () => {
    ...
    assert.ok(
      validatorResult(
        validators.array(validators.hex)([["0x0", "0x1", "0x2"]], 0),
      ),
    );
    assert.ok(
      validatorResult(
    ...
    assert.ok(
      validatorResult(validators.array(validators.bool)([[true, false]], 0)),
    );

    // invalid
    assert.notOk(
      validatorResult(
        validators.array(validators.hex)([["0x0", "0x1", "0x2", "true"]], 0),
      ),
    );
    assert.notOk(
      validatorResult(
    ...
    assert.notOk(
      validatorResult(
        validators.array(validators.blockHash)([["0xb6dbbc1cd0ace066b3"]], 0),
      ),
    );
    assert.notOk(
      validatorResult(
        validators.array(validators.bool)([["0x123", "0x456", "0x789"]], 0),
      ),
    );
    assert.notOk(
      validatorResult(validators.array(validators.bool)([[true, "true"]], 0)),
    );
  });
  ...
  it("rewardPercentiles", () => {
    ...
    assert.ok(
      validatorResult(validators.rewardPercentiles([[0, 2, 5, 30, 100]], 0)),
    );
    assert.ok(
      validatorResult(
        validators.rewardPercentiles([[0, 2.1, 5.35, 30.999, 60, 100]], 0),
      ),
    );
    ...
    assert.notOk(
      validatorResult(validators.rewardPercentiles([[1, 2, 3, 2.5]], 0)),
  ...
  it("values", () => {
    // valid
    assert.ok(
      validatorResult(validators.values(["VALID", "INVALID"])(["VALID"], 0)),
    );
    assert.ok(
      validatorResult(validators.values(["VALID", "INVALID"])(["INVALID"], 0)),
    );

    // invalid
    assert.notOk(
      validatorResult(validators.values(["VALID", "INVALID"])(["ANOTHER"], 0)),
    );
    assert.notOk(
      validatorResult(validators.values(["VALID", "INVALID"])(["valid"], 0)),
    );
  ...
  it("optional", () => {
    ...
    assert.ok(
      validatorResult(validators.optional(validators.blockHash)([""], 0)),
    );
    assert.ok(
      validatorResult(validators.optional(validators.blockHash)([], 0)),
    );
    ...
    assert.ok(
      validatorResult(
        validators.optional(validators.values(["VALID", "INVALID"]))(
          ["INVALID"],
          0,
        ),
      ),
    ...
    assert.ok(
      validatorResult(
        validators.optional(validators.values(["VALID", "INVALID"]))([""], 0),
      ),
    );
    assert.ok(
      validatorResult(
    ...
    // invalid
    assert.notOk(
      validatorResult(validators.optional(validators.bool)(["hey"], 0)),
    );
    assert.notOk(
      validatorResult(validators.optional(validators.blockHash)(["0x0"], 0)),
    );
    assert.notOk(
      validatorResult(
        validators.optional(validators.values(["VALID", "INVALID"]))(
  ...
  it("either", () => {
    ...
    assert.ok(
      validatorResult(
        validators.either(validators.bool, validators.blockHash)([true], 0),
      ),
    );
    assert.ok(
      validatorResult(
        validators.either(validators.bool, validators.hex)(["0xaaa"], 0),
      ),
    );
    assert.ok(
      validatorResult(
    ...
    assert.notOk(
      validatorResult(
        validators.either(validators.bool, validators.blockHash)(["0xabc"], 0),
      ),
    );
    assert.notOk(
      validatorResult(
        validators.either(validators.bool, validators.hex)(["abc"], 0),
      ),
    );
    assert.notOk(
      validatorResult(
        validators.either(validators.hex, validators.blockHash)([true], 0),
      ),
    );
    assert.notOk(
      validatorResult(
