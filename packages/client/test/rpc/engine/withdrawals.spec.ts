import { genWithdrawalsTrieRoot } from "@theqrl/zondjs-block";
...
const withdrawalsVector = [
  ...
  },
  {
    ...
    Amount:
      "3166189940082864718613269121331309980362851143201109172953918312716374638592",
  },
];
...
const testCases = [
  ...
  },
  {
    ...
    withdrawalsRoot:
      "0xb695b29ec7ee934ef6a68838b13729f2d49fffe26718de16a1a9ed94a4d7d06d",
    gethBlockRlp: gethWithdrawals8BlockRlp,
  },
];

for (const { name, withdrawals, withdrawalsRoot, gethBlockRlp } of testCases) {
  const validPayloadAttributesWithWithdrawals = {
  ...
  it(name, async () => {
    ...
    const computedWithdrawalsRoot = bytesToHex(
      await genWithdrawalsTrieRoot(
        withdrawals.map(createWithdrawal),
    ...
    const { server } = await setupChain(withdrawalsData, "post-merge", {
      engine: true,
    });
