...
describe(method, () => {
  ...
  it("call with invalid hex string as block hash", async () => {
    ...
    const blockDataWithInvalidBlockHash = [
      { ...blockData, blockHash: "0x-invalid-block-hash" },
    ];
  ...
  it("call with non existent block hash", async () => {
    const { server } = await setupChain(postMergeData, "merge", {
      engine: true,
    });
    ...
    const blockDataNonExistentBlockHash = [
      {
        ...blockData,
        blockHash:
          "0x2559e851470f6e7bbed1db474980683e8c315bfce99b2a6ef47c057c04de7858",
      },
    ];
    ...
    const blockDataNonExistentParentHash = [
      {
        ...
        parentHash:
          "0x2559e851470f6e7bbed1db474980683e8c315bfce99b2a6ef47c057c04de7858",
        blockHash:
          "0xf31969a769bfcdbcc1c05f2542fdc7aa9336fc1ea9a82c4925320c035095d649",
      },
    ];
  ...
  it("call with valid data but invalid transactions", async () => {
    const { chain, server } = await setupChain(postMergeData, "post-merge", {
      engine: true,
    });
    const rpc = getRPCClient(server);
  ...
  it("call with valid data & valid transaction but not signed", async () => {
    const { server, common, chain } = await setupChain(
      postMergeData,
      "post-merge",
      {
        engine: true,
    ...
    const blockDataWithValidTransaction = {
      ...
      transactions,
      blockHash:
        "0x308f490332a31fade8b2b46a8e1132cd15adeaffbb651cb523c067b3f007dd9e",
    };
    ...
    assert.equal(res.result.status, "INVALID");
    assert.isTrue(
      res.result.validationError.includes("transaction at index 0 is unsigned"),
    );
  });
  ...
  it("call with valid data & valid transaction", async () => {
    ...
    const { server, common } = await setupChain(newGenesisJSON, "post-merge", {
      engine: true,
    });
    const rpc = getRPCClient(server);
    const tx = createFeeMarket1559Tx(
    ...
    const blockDataWithValidTransaction = {
      ...
      transactions,
      parentHash:
        "0xefc1993f08864165c42195966b3f12794a1a42afa84b1047a46ab6b105828c5c",
      receiptsRoot:
        "0xc508745f9f8b6847a127bbc58b7c6b2c0f073c7ca778b6f020138f0d6d782adf",
      gasUsed: "0xcf08",
      stateRoot:
        "0x5a7123ab8bdd4f172438671a2a3de143f2105aa1ac3338c97e5f433e8e380d8d",
      blockHash:
        "0x625f2fd36bf278f92211376cbfe5acd7ac5da694e28f3d94d59488b7dbe213a4",
    };
  ...
  it("call with too many transactions", async () => {
    ...
    const { chain, server, common } = await setupChain(
      newGenesisJSON,
      "post-merge",
      {
        engine: true,
    ...
    const blockDataWithValidTransaction = {
      ...
      transactions,
      parentHash:
        "0x7444bd276e83a1ad90cf2ce8d1cff9ad15ed2489e49928a802bd60e3e81010f4",
      receiptsRoot:
        "0x29651db7ce551aae097d75c4c9785e55068a11cf9e365bbe1c3b1780a85621c0",
      gasUsed: "0x51ae28",
      stateRoot:
        "0xd51a194147c26671af9ce46e9f5914d3e64fac15e80e46be5cc8c42c935449bc",
      blockHash:
        "0x7c5cc6138adca74b80e6066c30e330b6307ff43bc0dc3dd4e988bb1b9764d199",
    };
  ...
  it("parent hash equals to block hash", async () => {
    ...
    const res = await rpc.request(
      method,
      blockDataHasBlockHashSameAsParentHash,
    );
