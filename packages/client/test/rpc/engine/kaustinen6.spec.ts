...
async function fetchExecutionPayload(
  ...
  try {
    const beaconBlock = await (
      await fetch(`${peerBeaconUrl}/eth/v2/beacon/blocks/${slot}`)
    ).json();
...
async function runBlock(
  ...
  const parentPayload =
    isBeaconData === true
      ? executionPayloadFromBeaconPayload(parent as any)
      : parent;
  ...
  const executePayload =
    isBeaconData === true
      ? executionPayloadFromBeaconPayload(execute as any)
      : execute;
...
describe.skip(`valid verkle network setup`, async () => {
  const { server, chain, common } = await setupChain(
    kaustinen6Data,
    "post-merge",
    {
      engine: true,
  ...
  for (const numberOrRange of process.env.TEST_ONLINE_SLOTS?.split(",") ?? []) {
    ...
    for (let i = startSlot; i <= endSlot; i++) {
      const execute = await fetchExecutionPayload(
        process.env.PEER_BEACON_URL,
        i,
      );
...
async function loadGethVectors(
  vectorsDirPath: string,
  opts: { common: Common },
) {
  // set chain id to 1 for geth vectors
  opts.common["_chainParams"].chainId = 1;
  const stateDiffVec = JSON.parse(
    readFileSync(`${vectorsDirPath}/statediffs.json`, "utf8"),
  );
  ...
  const block0RlpHex = readFileSync(
    `${vectorsDirPath}/block0.rlp.hex`,
    "utf8",
  ).trim();
  ...
  const block1RlpHex = readFileSync(
    `${vectorsDirPath}/block1.rlp.hex`,
    "utf8",
  ).trim();
  ...
  const block2RlpHex = readFileSync(
    `${vectorsDirPath}/block2.rlp.hex`,
    "utf8",
  ).trim();
