...
describe(method, () => {
  ...
  it("call with valid data but parent block is not loaded yet", async () => {
    ...
    const nonExistentHeadBlockHash = {
      ...validForkChoiceState,
      headBlockHash:
        "0x1d93f244823f80efbd9292a0d0d72a2b03df8cd5a9688c6c3779d26a7cc5009c",
    };
    const res = await rpc.request(method, [
      nonExistentHeadBlockHash,
      validPayloadAttributes,
    ]);
    ...
    const res = await rpc.request(method, [
      {
        ...validForkChoiceState,
        finalizedBlockHash:
          "0x3b8fb240d288781d4aac94d3fd16809ee413bc99294a085798a589dae51ddd4b",
      },
    ]);
    assert.equal(res.error.code, INVALID_FORKCHOICE_STATE);
    assert.ok(
      res.error.message.includes(
        "finalized block not available in canonical chain",
      ),
    );
  });
  ...
  it("latest block after reorg", async () => {
    const { server, blockchain } = await setupChain(
      postMergeData,
      "post-merge",
      {
        engine: true,
    ...
    assert.equal(
      res.result.number,
      "0x0",
      "finalized should be set to genesis",
    );
  ...
  it("validate safeBlockHash is part of canonical chain", async () => {
    const { server, chain } = await setupChain(postMergeData, "post-merge", {
      engine: true,
    });
    const rpc = getRPCClient(server);
