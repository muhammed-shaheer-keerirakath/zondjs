...
describe(method, () => {
  ...
  it("get blobGasUsed/blobGasPrice in blob tx receipt", async () => {
    ...
    if (isBrowser() === true) {
      ...
      assert.equal(
        res.result.blobGasUsed,
        "0x20000",
        "receipt has correct blob gas usage",
      );
      assert.equal(
        res.result.blobGasPrice,
        "0x1",
        "receipt has correct blob gas price",
      );

      const res2 = await rpc.request(method2, [
      ...
      assert.deepEqual(
        res2.result,
        [res.result],
        "transaction result is 1 since succeeded",
      );
    }
  });
});
