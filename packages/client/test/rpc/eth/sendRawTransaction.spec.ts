...
describe(method, () => {
  ...
  it("send local tx with gasprice lower than minimum", async () => {
    ...
    const syncTargetHeight = new Common({ chain: Mainnet }).hardforkBlock(
      Hardfork.London,
    );
  ...
  it("call with invalid tx (wrong chain ID)", async () => {
    ...
    assert.ok(
      res.error.message.includes("serialized tx data could not be parsed"),
    );
  });
  ...
  it("blob EIP 4844 transaction", async () => {
    ...
    const consensusFormatValidation =
      BlockHeader.prototype["_consensusFormatValidation"];
    ...
    const res = await rpc.request(method, [
      bytesToHex(tx.serializeNetworkWrapper()),
    ]);

    const res2 = await rpc.request(method, [
      bytesToHex(replacementTx.serializeNetworkWrapper()),
    ]);
