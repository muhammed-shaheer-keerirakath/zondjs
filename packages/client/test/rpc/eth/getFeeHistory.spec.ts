...
const produceFakeGasUsedBlock = async (
...
describe(method, () => {
  it(`${method}: should return 12.5% increased baseFee if parent block is full`, async () => {
    ...
    const [firstBaseFee, previousBaseFee, nextBaseFee] = res.result
      .baseFeePerGas as [string, string, string];
    const increase =
      Number(
        (1000n *
          (bytesToBigInt(hexToBytes(nextBaseFee)) -
  ...
  it(`${method}: should return correct gas used ratios`, async () => {
    ...
    const [genesisGasUsedRatio, nextGasUsedRatio] = res.result.gasUsedRatio as [
      number,
      number,
    ];
  ...
  it(`${method}: should generate reward percentiles`, async () => {
    ...
    assert.ok(
      res.result.reward[0].length > 0,
      "Produced at least one rewards percentile",
    );
  });
  ...
  it(`${method}: should generate reward percentiles - sorted check`, async () => {
    ...
    await produceBlockWithTx(
      execution,
      chain,
      priorityFees.reverse(),
      gasUsed.reverse(),
    );
  ...
  it(
    ...
    async () => {
      const { chain, execution, server } = await setupChain(
        eip4844Data,
        "post-merge",
        {
          engine: true,
      ...
      expBlobGas.push(
        bigIntToHex(head.header.calcNextBlobGasPrice(head.common)),
      );
