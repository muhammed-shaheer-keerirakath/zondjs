...
import { createBlob4844Tx, createLegacyTx } from "@theqrl/zondjs-tx";
...
function createChain(headBlock = block) {
  ...
  const genesisBlock = {
    ...
    serialize: () =>
      createBlock({ header: { number: 0 }, transactions }).serialize(),
    header: {
      number: BigInt(0),
    },
    toJSON: () => ({
...
describe(method, async () => {
  ...
  it("call with earliest param", async () => {
    ...
    assert.equal(
      res.result.number,
      "0x0",
      "should return the genesis block number",
    );
  });
  ...
  it("call with non-string block number", async () => {
    ...
    assert.ok(
      res.error.message.includes(
        "invalid argument 0: argument must be a string",
      ),
    );
  });
  ...
  it("call without second parameter", async () => {
    ...
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
      res.error.message.includes("missing value for required argument 1"),
    );
  });
  ...
  describe("call with block with blob txs", () => {
    it("retrieves a block with a blob tx in it", async () => {
      ...
      const manager = createManager(
        await createClient({ chain: createChain(block1 as any) }),
      );
      const rpc = getRPCClient(startRPC(manager.getMethods()));
      const res = await rpc.request(method, ["latest", true]);
