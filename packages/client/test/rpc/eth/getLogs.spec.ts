...
describe(method, async () => {
  it("call with valid arguments", async () => {
    ...
    if (
      ...
      res.result.every((r: any) => r.address === contractAddr1.toString()) ===
        true
    ) {
      assert.ok(
        true,
        "should return the correct logs (filter by single address)",
      );
    } else {
    ...
    if (
      ...
      assert.ok(
        true,
        "should return the correct logs (filter by multiple addresses)",
      );
    } else {
      assert.fail(
        "should return the correct logs (filter by multiple addresses)",
