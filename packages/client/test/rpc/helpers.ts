...
export async function createClient(clientOpts: Partial<createClientArgs> = {}) {
  ...
  const genesisState =
    clientOpts.genesisState ?? getGenesis(Number(common.chainId())) ?? {};
  ...
  const blockchain =
    clientOpts.blockchain ?? (mockBlockchain() as unknown as Blockchain);

  const chain =
    clientOpts.chain ??
    (await Chain.create({ config, blockchain, genesisState }));
  chain.opened = true;
  ...
  if (!(clientOpts.includeVM === false)) {
    const metaDB: any =
      clientOpts.enableMetaDB === true ? new MemoryLevel() : undefined;
...
export async function baseSetup(clientOpts: any = {}) {
  ...
  const engineMethods =
    clientOpts.engine === true ? manager.getMethods(true, true) : {};
...
export async function setupChain(
  ...
  const genesisMeta = common.gteHardfork(Hardfork.Verkle)
    ? { genesisStateRoot }
    : { genesisState };
  ...
  const manager = createManager(client);
  const engineMethods =
    clientOpts.engine === true ? manager.getMethods(true) : {};
...
export function gethGenesisStartLondon(gethGenesis: any) {
  ...
  return {
    ...gethGenesis,
    config: { ...gethGenesis.config, ...Object.fromEntries(londonConfig) },
  };
}
