...
describe(method, () => {
  ...
  beforeEach<TestSetup>(async (context) => {
    ...
    const result = await blockBuilder.addTransaction(firstTx, {
    ...
    await blockBuilder.addTransaction(secondTx, {
      skipHardForkValidation: true,
    });
    ...
    const thirdResult = await blockBuilder.addTransaction(thirdTx, {
      skipHardForkValidation: true,
    });
  ...
  it<TestSetup>("Should provide a valid nextKey if there is one.", async ({
    ...
    const largestHashedKey = bytesToHex(
      keccak256(setLengthLeft(hexToBytes("0x01"), 32)),
    );

    const storageRange: StorageRange = res.result;

    assert.equal(
      storageRange.nextKey,
      largestHashedKey,
      "nextKey was correctly set.",
    );
  });
  ...
  it<TestSetup>("Should provide a null value for preimages (until this is implemented).", async ({
    ...
    for (const [_, KVPair] of Object.entries(storageRange.storage)) {
      assert.isNull(
        KVPair.key,
        "Storage key preimage was intentionally set to null.",
      );
    }
  });
  ...
  it<TestSetup>("Should throw an error if the requested block does not exist.", async ({
    ...
    assert.ok(
      res.error.message.includes("Could not get requested block hash."),
    );
  });
  ...
  it<TestSetup>("Should throw an error if txIndex is too small or too large.", async ({
    ...
    assert.ok(
      res.error.message.includes(
        "invalid argument 1: argument must be larger than 0",
      ),
    );
  ...
  it<TestSetup>("Should throw an error if the address is not formatted correctly.", async ({
    ...
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
      res.error.message.includes("invalid argument 2: invalid address"),
    );
  });
  ...
  it<TestSetup>("Should throw an error if hex parameters do not start with '0x'.", async ({
    ...
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
      res.error.message.includes("invalid argument 2: missing 0x prefix"),
    );
    ...
    assert.ok(
      res.error.message.includes(
        "invalid argument 3: hex string without 0x prefix",
      ),
    );
  });
});
