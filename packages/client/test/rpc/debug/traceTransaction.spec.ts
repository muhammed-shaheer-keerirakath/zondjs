...
describe(method, () => {
  ...
  it("call with invalid parameters", async () => {
    ...
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
      res.error.message.includes("enabling return data not implemented"),
    );

    res = await rpc.request(method, [
      "0xabcd",
      { tracerConfig: { some: "value" } },
    ]);
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
    ...
    assert.equal(res.error.code, INVALID_PARAMS);
    assert.ok(
      res.error.message.includes("custom tracer timeouts not implemented"),
    );
  });
  ...
  it("call with reverting code", async () => {
    ...
    assert.equal(
      res.result.failed,
      true,
      "returns error result with reverting code",
    );
  });
  ...
  it("call with stack disabled", async () => {
    ...
    assert.isUndefined(
      res.result.structLogs[1].stack,
      "returns no stack with trace",
    );
  });
});
