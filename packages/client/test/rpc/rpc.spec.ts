...
describe("JSON-RPC call", () => {
  ...
  it("auth protected server with an invalid algorithm token", async () => {
    ...
    try {
      ...
      assert.equal(
        err.message,
        "Unauthorized: Error: Signature verification failed",
      );
    }
  });
  ...
  it("auth protected server with a valid token", async () => {
    ...
    try {
      ...
      assert.fail(
        "should have returned a valid response for an unknown method",
      );
    }
  });
  ...
  it("auth protected server with unprotected method without token", async () => {
    ...
    try {
      const res = await rpc.request("unprotected_METHOD_DOES_NOT_EXIST", [
        "0x1",
        true,
      ]);
      assert.equal(res.error.message, "Method not found");
  ...
  it("auth protected server with protected method without token", async () => {
    const server = startRPC({}, undefined, {
      jwtSecret,
      unlessFn: (req: any) =>
        !(req.body.method as string).includes("protected_"),
    });
    ...
    try {
      ...
      assert.ok(
        err.message.includes("Missing auth header"),
  ...
  it("auth protected server with protected method with token", async () => {
    ...
    try {
      const res = await rpc.request("protected_METHOD_DOES_NOT_EXIST", [
        "0x1",
        true,
      ]);
      assert.equal(res.error.message, "Method not found");
