...
describe(
  ...
  async () => {
    ...
    const [reqId1, headers] = await peer.eth!.getBlockHeaders({
      block: BigInt(1),
      max: 2,
    });
    it("handled getBlockHeaders", async () => {
      ...
      assert.ok(
        equalsBytes(headers![1].hash(), hash),
    ...
    service.config.chainCommon.getHardforkBy =
      td.func<typeof config.chainCommon.getHardforkBy>();
    td.when(
      service.config.chainCommon.getHardforkBy(td.matchers.anything()),
    ).thenReturn(Hardfork.London);
    const [_, txs] = await peer.eth!.getPooledTransactions({
      hashes: [tx.hash()],
    });
    it("should handle GetPooledTransactions", async () => {
      assert.ok(
        equalsBytes(txs[0].hash(), tx.hash()),
