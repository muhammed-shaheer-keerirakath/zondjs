...
async function minerSetup(): Promise<EthereumClient[]> {
  ...
  const miner = await createInlineClient(
    config1,
    common,
    customGenesisState,
    "",
  ...
  const config2 = new Config({
    ...
    accountCache: 10000,
    storageCache: 1000,
    bootnodes: parseMultiaddrs(
      miner.config.server!.getRlpxInfo().enode as string,
    ),
    accounts,
  ...
  const follower = await createInlineClient(
    config2,
    common,
    customGenesisState,
    "",
