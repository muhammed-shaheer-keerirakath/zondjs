...
const { FullEthereumService } = await import(
  "../../src/service/fullethereumservice.js"
);
...
describe("initialize", async () => {
  ...
  it("should get protocols", async () => {
    ...
    assert.ok(
      service.protocols.filter((p) => p.name === "eth").length > 0,
      "full protocol",
    );
    assert.notOk(
      service.protocols.filter((p) => p.name === "les").length > 0,
      "no light protocol",
    );
    config = new Config({});
...
describe("should call handleNewBlock on NewBlock and handleNewBlockHashes on NewBlockHashes", async () => {
  ...
  it("should handle new blockhashes", async () => {
    ...
    expect((service.synchronizer as any).handleNewBlockHashes).toBeCalledWith([
      {},
      BigInt(1),
    ]);
  });
...
describe("should ban peer for sending NewBlock/NewBlockHashes after merge", async () => {
  ...
  service.pool.ban = () => {
    it("should ban peer", () => {
      assert.ok(
        true,
        "banned peer when NewBlock/NewBlockHashes announced after Merge",
      );
    });
  };

  await service.handle({ name: "NewBlock", data: [{}, BigInt(1)] }, "eth", {
    id: 1,
  } as any);
  await service.handle({ name: "NewBlockHashes", data: [] }, "eth", {
    id: 1,
  } as any);
});
...
describe("should send Receipts on GetReceipts", async () => {
  ...
  it("should handle getReceipts", async () => {
    await service.handle(
      { name: "GetReceipts", data: [BigInt(1), [blockHash]] },
      "eth",
      peer,
    );
    expect(peer.eth.send).toBeCalledWith("Receipts", {
      reqId: BigInt(1),
      receipts,
    });
  });
  ...
  service.txPool.handleAnnouncedTxHashes = async (msg, _peer, _pool) => {
    it("should handle NewPooledTransactionHashes", () => {
      assert.deepEqual(
        msg[0],
        hexToBytes("0xabcd"),
        "handled NewPooledTransactionHashes",
      );
    });
  };
...
describe("should start on beacon sync when past merge", async () => {
  ...
  it("should not be available", () => {
    const configDisableBeaconSync = new Config({
      common,
      syncmode: SyncMode.None,
    });
    const service = new FullEthereumService({
      config: configDisableBeaconSync,
      chain,
    });
