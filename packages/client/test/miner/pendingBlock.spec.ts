...
describe("[PendingBlock]", async () => {
  ...
  it("should start and build", async () => {
    ...
    assert.equal(
      block?.header.number,
      BigInt(1),
      "should have built block number 1",
    );
    assert.equal(block?.transactions.length, 3, "should include txs from pool");
    assert.equal(
      receipts.length,
      3,
      "receipts should match number of transactions",
    );
  ...
  it("should include txs with mismatching hardforks that can still be executed", async () => {
    ...
    assert.equal(
      (payload as any).transactions.filter(
        (tx: TypedTransaction) =>
          bytesToHex(tx.hash()) === bytesToHex(txA011.hash()),
      ).length,
      1,
    ...
    assert.equal(
      (payload as any).transactions.filter(
        (tx: TypedTransaction) =>
          bytesToHex(tx.hash()) === bytesToHex(txB011.hash()),
      ).length,
      1,
  ...
  it("construct blob bundles", async () => {
    ...
    const blobGasLimit =
      txPool["config"].chainCommon.param("maxBlobGasPerBlock");
    ...
    const allowedLength =
      allowedBlobsPerBlock * txPool["config"].blobsAndProofsCacheBlocks;
    ...
    const payloadId = await pendingBlock.start(vm, parentBlock);
    const [block, _receipts, _value, blobsBundles] =
    ...
    assert.equal(
      block!.transactions.length,
      2,
      "Only two blob txs should be included",
    );
    assert.equal(
      blobsBundles!.blobs.length,
      6,
      "maximum 6 blobs should be included",
    );
    assert.equal(
    ...
    assert.equal(
      blobsBundles!.proofs.length,
      6,
      "maximum 6 proofs should be included",
    );
  ...
  it("should exclude missingBlobTx", async () => {
    ...
    assert.equal(
      block!.transactions.length,
      0,
      "Missing blob tx should not be included",
    );
  });
});
