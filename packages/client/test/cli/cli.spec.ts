...
describe("[CLI]", () => {
  ...
  it("should successfully start client with custom inputs for PoA network", async () => {
    ...
    const onData = async (
      ...
      if (message.includes("Client started successfully")) {
        assert.ok(
          message,
          "Client started successfully with custom inputs for PoA network",
        );
        count -= 1;
      }
      if (count === 0) {
  ...
  it("should start engine rpc on custom address", async () => {
    ...
    const onData = async (
      ...
      if (message.includes("http://")) {
        ...
        assert.ok(
          res.result.length > 0,
          "engine api is responsive on custom address",
        );
        child.kill();
        resolve(undefined);
      }
    ...
    const onData = async (
      ...
      if (message.includes("ws://") && message.includes("engine")) {
        ...
        (client as any).ws.on("open", async function () {
          ...
          assert.ok(
            res.result.length > 0,
            "read from WS RPC on custom address and port",
          );
          child.kill();
          resolve(undefined);
        });
    ...
    const onData = async (
      ...
      if (message.includes("http://")) {
        ...
        try {
          ...
          assert.ok(
            e !== undefined,
            "failed to connect to RPC on invalid address",
          );
          child.kill();
          resolve(undefined);
        }
  ...
  it("should start client with file path for bootnodes option", async () => {
    ...
    const onData = async (
      ...
      if (message.includes("Reading bootnodes")) {
        assert.ok(
          message.includes("num=2"),
          "passing bootnode.txt URL for bootnodes option works",
        );
        child.kill();
        resolve(undefined);
      }
  ...
  it("should start client with custom sync parameters", async () => {
    ...
    const onData = async (
      ...
      if (message.includes("Starting FullEthereumService")) {
        assert.ok(
          message.includes("with no syncing"),
          "client respects custom sync mode option",
        );
      }
      if (message.includes("Client started successfully")) {
        assert.ok(
    ...
    const onData = async (
      ...
      if (message.includes("Data directory")) {
        assert.ok(
          ...
          "client respects custom data directory option",
        );
      }
      if (message.includes("Initializing Ethereumjs client")) {
        assert.ok(
  ...
  it("should not start client with conflicting parameters", async () => {
    ...
    const onData = async (
      ...
      if (
        message.includes(
          "Arguments chainId and gethGenesis are mutually exclusive",
        )
      ) {
...
describe("verkle execution", () => {
 ...
 `;
    ...
    const onData = async (
      ...
      if (
        message.includes("Setting up verkleVM for stateful verkle execution")
      ) {
