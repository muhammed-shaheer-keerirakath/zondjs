...
describe("[Config]", () => {
  ...
  it("Chain data default directory", () => {
    const config = new Config();
    assert.equal(
      config.getDataDirectory(DataDirectory.Chain),
      "./datadir/mainnet/chain",
    );
  });
  ...
  it("State data default directory", () => {
    const config = new Config();
    assert.equal(
      config.getDataDirectory(DataDirectory.State),
      "./datadir/mainnet/state",
    );
  });
  ...
  it("Meta data default directory", () => {
    const config = new Config();
    assert.equal(
      config.getDataDirectory(DataDirectory.Meta),
      "./datadir/mainnet/meta",
    );
  });
  ...
  it("peer discovery default mainnet setting", () => {
    ...
    assert.equal(
      config.discDns,
      false,
      "disables DNS peer discovery for mainnet",
    );
  ...
  it("--discDns=true/false", () => {
    ...
    assert.equal(
      config.discDns,
      true,
      `default discDns setting can be overridden to true`,
    );
    ...
    assert.equal(
      config.discDns,
      false,
      `default discDns setting can be overridden to false`,
    );
  });
  ...
  it("--discV4=true/false", () => {
    ...
    assert.equal(
      config.discV4,
      false,
      `default discV4 setting can be overridden to false`,
    );
    ...
    assert.equal(
      config.discV4,
      true,
      `default discV4 setting can be overridden to true`,
    );
  });
});
