...
describe("POW tests", () => {
  it("should work", async () => {
    for (const key of tests) {
      ...
      const header = createBlockHeaderFromRLP(hexToBytes(`0x${test.header}`), {
        common,
      });

      const headerHash = ethash.headerHash(header.raw());
      assert.equal(
        bytesToHex(headerHash),
        "0x" + test.header_hash,
      ...
      assert.equal(
        await getCacheSize(epoc),
        test.cache_size,
        "generate cache size",
      );
      assert.equal(
        await getFullSize(epoc),
        test.full_size,
