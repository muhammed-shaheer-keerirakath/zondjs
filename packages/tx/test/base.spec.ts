...
describe("[BaseTransaction]", () => {
  ...
  for (const tx of eip2930TxsData) {
    eip2930Txs.push(
      createAccessList2930Tx(tx.data as AccessList2930TxData, { common }),
    );
  }
  ...
  for (const tx of eip1559TxsData) {
    eip1559Txs.push(
      createFeeMarket1559Tx(tx.data as FeeMarketEIP1559TxData, { common }),
    );
  }
  ...
  it("Initialization", () => {
    for (const txType of txTypes) {
      ...
      assert.ok(
        Object.isFrozen(tx),
        `${txType.name}: tx should be frozen by default`,
      );
  ...
  it("createWithdrawalFromBytesArray()", () => {
    ...
    try {
      ...
      assert.fail(
        "should have thrown when maxPriorityFeePerGas has leading zeroes",
      );
    } catch (err: any) {
      assert.ok(
