    ...
    for (const value of values) {
      ...
      for (const testCase of cases) {
        if (
          !(
            value === "chainId" &&
            ((typeof testCase === "number" && isNaN(<number>testCase)) ||
              testCase === false)
          )
        ) {
  ...
  it("Access Lists -> success cases", () => {
    for (const txType of txTypes) {
      ...
      assert.deepEqual(
        JSON,
        access,
        `should allow json-typed access lists (${txType.name})`,
      );
  ...
  it("getDataGas()", () => {
    for (const txType of txTypes) {
      ...
      assert.equal(
        tx.getDataGas(),
        BigInt(0),
        "Should return data fee when frozen",
      );
      ...
      assert.equal(
        tx.getDataGas(),
        BigInt(0),
        "Should return data fee when not frozen",
      );

      const mutableCommon = new Common({
        chain: Mainnet,
        hardfork: Hardfork.London,
      });
...
describe("[AccessList2930Tx] -> Class Specific Tests", () => {
  ...
  it(`should return right upfront cost`, () => {
    ...
    const accessListStorageKeyCost: number = Number(
      common.param("accessListStorageKeyGas"),
    );
    const accessListAddressCost: number = Number(
      common.param("accessListAddressGas"),
    );
    ...
    assert.ok(
      tx.getIntrinsicGas() ===
        BigInt(
          baseFee + accessListAddressCost * 2 + accessListStorageKeyCost * 3,
        ),
    );
  });
  ...
  it("unsigned tx -> getHashedMessageToSign()/getMessageToSign()", () => {
    ...
    assert.deepEqual(unsignedTx.getHashedMessageToSign(), expectedHash),
      "correct hashed version";
  ...
  it("common propagates from the common of tx, not the common in TxOptions", () => {
    ...
    assert.notDeepEqual(
      newCommon,
      common,
      "new common is different than original common",
    );
