...
describe("EIP4844 constructor tests - valid scenarios", () => {
  ...
  it("should work", () => {
    ...
    assert.equal(
      tx.type,
      3,
      "successfully instantiated a blob transaction from txData",
    );
    const factoryTx = createTx(txData, { common });
    assert.equal(
      factoryTx.type,
      3,
      "instantiated a blob transaction from the tx factory",
    );
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      if (hashSet.has(hash)) {
        assert.ok(
          false,
          "should not reuse the same hash (hedged signature test)",
        );
      }
      hashSet.add(hash);
    }
...
describe("Network wrapper tests", () => {
  ...
  it("should work", async () => {
    ...
    const deserializedTx = createBlob4844TxFromSerializedNetworkWrapper(
      wrapper,
      {
        common,
      },
    ...
    assert.ok(
      minimalTx.blobs === undefined,
      "minimal representation contains no blobs",
    );
    assert.ok(
    ...
    const serializedWithMissingBlob =
      txWithMissingBlob.serializeNetworkWrapper();

    assert.throws(
      () =>
        createBlob4844TxFromSerializedNetworkWrapper(
          serializedWithMissingBlob,
          {
            common,
          },
        ),
    ...
    const serializedWithInvalidCommitment =
      txWithInvalidCommitment.serializeNetworkWrapper();
    ...
    blobVersionedHashes[0] = ("0x0102" +
      blobVersionedHashes[0].slice(6)) as PrefixedHexString;
    ...
    const unsignedTx = createBlob4844Tx(
      {
        ...
        blobVersionedHashes: [
          hexToBytes(
            "0x01624652859a6e98ffc1608e2af0147ca4e86e1ce27672d8d3f3c9d4ffd6ef7e",
          ),
        ],
...
describe("Network wrapper deserialization test", () => {
  ...
  it("should work", async () => {
    const txData = {
      ...
      blobVersionedHashes: [
        "0x0172ff1d4f354eebdb3cd0cb64e41ac584359094373fd5f979bcccbd6072d936",
      ],
      yParity: "0x0",
    };
    const txMeta = {
      ...
      unsignedHash:
        "0x3103e9d2b39b26082a6e36fa0364e5b14502efa0041ad29d485737a10e03431a",
    ...
    const networkSerialized = bytesToHex(
