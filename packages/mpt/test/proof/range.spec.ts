...
async function verify(
  ...
  return verifyMerkleRangeProof(
    ...
    [
      ...(await createMerkleProof(trie, startKey)),
      ...(await createMerkleProof(trie, endKey)),
    ],
  );
}
...
describe("simple merkle range proofs generation and verification", () => {
  it("create a range proof and verify it", async () => {
    ...
    assert.equal(
      await verify(trie, entries, entries.length - 2, entries.length - 1),
      false,
    );
  });
  ...
  it("create a non-existent range proof and verify it", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (
        endKey === undefined ||
        (end < entries.length - 1 &&
          compareBytes(endKey, entries[end + 1][0]) >= 0)
      ) {
        continue;
      }
    ...
    assert.equal(
      await verify(trie, entries, 0, entries.length - 1, startKey, endKey),
      false,
    );
  });
  ...
  it("create invalid non-existent range proof and verify it", async () => {
    ...
    assert.equal(
      await verify(trie, entries, start, end, decreasedStartKey, endKey),
      true,
    );
    try {
    ...
    assert.equal(
      await verify(trie, entries, start, end, startKey, increasedEndKey),
      true,
    );
    try {
  ...
  it("create a one element range proof and verify it", async () => {
    ...
    assert.equal(
      await verify(trie, entries, start, start, decreasedStartKey),
      true,
    );

    // One element with right non-existent edge proof
    assert.equal(
      await verify(trie, entries, start, start, undefined, increasedEndKey),
      true,
    );
  ...
  it("create a single side range proof and verify it", async () => {
    ...
    for (const end of cases) {
      assert.equal(
        await verify(trie, entries, 0, end, startKey),
        end !== entries.length - 1,
      );
    }
  });
  ...
  it("create a bad range proof and verify it", async () => {
    ...
    await runTest(async (trie, entries) => {
      ...
      entries = entries
        .slice(0, targetIndex)
        .concat(entries.slice(targetIndex + 1));
      await verify(trie, entries, start, end);
    });
  ...
  it("create a bloated range proof and verify it", async () => {
    ...
    for (let i = 0; i < TRIE_SIZE; i++) {
      bloatedProof = bloatedProof.concat(
        await createMerkleProof(trie, entries[i][0]),
      );
    }
