  ...
  for (const cacheSize of [0, 100]) {
    ...
    describe("setting back state root (deleteFromDB)", () => {
      it("should work", async () => {
         ...
         */
        const v1 = utf8ToBytes(
          "this-is-some-longer-value-to-test-the-delete-operation-value1",
        );
        const k2 = utf8ToBytes("2");
        const v2 = utf8ToBytes(
          "this-is-some-longer-value-to-test-the-delete-operation-value2",
        );
      ...
      it("should work", async () => {
        const useKeyHashingFunction: HashKeysFunction = (msg) => {
          ...
          if (msg.length <= hashLen - 5) {
            return concatBytes(
              utf8ToBytes("hash_"),
              new Uint8Array(hashLen - msg.length),
              msg,
            );
          } else {
