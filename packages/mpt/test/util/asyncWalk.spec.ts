...
describe("walk the tries from official tests", async () => {
  const testNames = Object.keys(
    trieTestData.tests,
  ) as (keyof typeof trieTestData.tests)[];
  ...
  for await (const testName of testNames) {
    ...
    describe(testName, async () => {
      ...
      for await (const [idx, input] of inputs.entries()) {
        const stringPair: [string, string] = [
          inputs[idx][0]!,
          inputs[idx][1] ?? "null",
        ];
...
describe("walk a sparse trie", async () => {
  ...
  const proofKey = isHexString(rawProofKey)
    ? hexToBytes(rawProofKey)
    : utf8ToBytes(rawProofKey);
  ...
  for await (const { currentKey, node } of walker) {
    if (
      equalsBytes((fromProof as any).hash(node.serialize()), fromProof.root())
    ) {
