...
for (const { constructor, defaults, title } of [
  ...
  if (IS_SECURE_TRIE) {
    EXPECTED_ROOTS =
      "0x8204723ce0fb452b130a282ecc727e07295c18cbd2c2eef33ba9eb9c7a9ded9b";
  } else {
    EXPECTED_ROOTS =
  ...
  describe(`${title} (Persistence)`, () => {
    it("creates an instance via createMPT and defaults to `false` with a database", async () => {
      assert.isFalse(
        (await createMPT({ ...defaults, db: new MapDB() }))["_opts"]
          .useRootPersistence,
      );
    });
    ...
    it("creates an instance via createMPT and defaults to `false` without a database", async () => {
      assert.isFalse(
        (await createMPT({ ...defaults }))["_opts"].useRootPersistence,
      );
    });
    ...
    it("persist the root if the `useRootPersistence` option is `true`", async () => {
      ...
      assert.equal(
        bytesToHex((await trie["_db"].get(ROOT_DB_KEY))!),
        EXPECTED_ROOTS,
      );
    });
    ...
    it("persist the root if the `root` option is given", async () => {
      ...
      assert.ok(
        equalsBytes((await trie["_db"].get(ROOT_DB_KEY))!, KECCAK256_RLP),
      );
    ...
    it("persists the root if the `db` option is not provided", async () => {
      ...
      await trie.put(
        utf8ToBytes("do_not_persist_without_db"),
        utf8ToBytes("bar"),
      );
    ...
    it("persist and restore the root", async () => {
      ...
      const copy = await createMPT({
        ...defaults,
        db,
        useRootPersistence: true,
      });
      assert.equal(
        bytesToHex((await copy["_db"].get(ROOT_DB_KEY))!),
        EXPECTED_ROOTS,
      );
    ...
    it("put fails if the key is the ROOT_DB_KEY", async () => {
      ...
      try {
        ...
        assert.equal(
          message,
          "Attempted to set '__root__' key but it is not allowed.",
        );
      }
    });
  });
  ...
  describe(`${title} (Check Root)`, async () => {
    ...
    it("should return true for all nodes in the trie", async () => {
      assert.isTrue(
        await trie.checkRoot(trie.root()),
        "Should return true for root node",
      );
    ...
    it("should return false for nodes not in the trie", async () => {
      for (let i = 0; i < 10; i++) {
        ...
        assert.isFalse(
          await trie.checkRoot(key),
          "Should return false for nodes not in trie",
        );
      }
    });
    it("should return false for all keys if trie is empty", async () => {
      ...
      assert.deepEqual(
        emptyTrie.EMPTY_TRIE_ROOT,
        emptyTrie.root(),
        "Should return empty trie root",
      );
      assert.isTrue(
...
describe("keyHashingFunction", async () => {
  it("uses correct hash function", async () => {
    ...
    const trieWithHashFunction = await createMPT({
    ...
    assert.equal(
      bytesToHex(trieWithCommon.root()),
      "0x80",
