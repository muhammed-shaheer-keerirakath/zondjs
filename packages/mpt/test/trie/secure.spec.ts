...
describe("secure tests", () => {
  ...
  it("empty values", async () => {
    for (const row of trieTestSecureTrieData.tests.emptyValues.in) {
      const val =
        row[1] !== undefined && row[1] !== null ? utf8ToBytes(row[1]) : null;
      await trie.put(utf8ToBytes(row[0]!), val);
    }
    assert.equal(
      bytesToHex(trie.root()),
      trieTestSecureTrieData.tests.emptyValues.root,
    );
  });
  ...
  it("branchingTests", async () => {
    ...
    assert.equal(
      bytesToHex(trie.root()),
      trieTestSecureTrieData.tests.branchingTests.root,
    );
  });
  ...
  it("jeff", async () => {
    ...
    assert.equal(
      bytesToHex(trie.root()),
      trieTestSecureTrieData.tests.jeff.root,
    );
  });
