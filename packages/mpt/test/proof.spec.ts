...
describe("simple merkle proofs generation and verification", () => {
  it("create a merkle proof and verify it", async () => {
    ...
    try {
      ...
      assert.fail(
        `expected error: 'The provided proof does not have the expected trie root'`,
      );
    } catch (e: any) {
      assert.equal(
        e.message,
  ...
  it("create a merkle proof and verify it whit keys in the middle", async () => {
    ...
    await trie.put(
      utf8ToBytes("key3"),
      utf8ToBytes("1234567890123456789012345678901"),
    );
  ...
  it("creates and updates tries from proof", async () => {
    ...
    assert.ok(
      equalsBytes(trieValue!, encodedValue),
      "trie value successfully copied",
    );
    ...
    assert.ok(
      equalsBytes(trieValue2!, encodedValue2),
      "trie value successfully updated",
    );
    assert.ok(equalsBytes(trie.root(), newTrie.root()), "root set correctly");
    ...
    assert.equal(
      newSafeValue,
      null,
      "cannot reach the trie item unless the root is set",
    );
