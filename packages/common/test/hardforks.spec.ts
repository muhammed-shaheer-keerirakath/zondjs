...
describe("[Common]: Hardfork logic", () => {
  it("Hardfork access", () => {
    ...
    for (const hardfork of supportedHardforks) {
      c = new Common({ chain: Mainnet, hardfork });
  ...
  it("getHardforkBy() / setHardforkBy()", () => {
    ...
    assert.equal(
      c.getHardforkBy({ blockNumber: 1149999n }),
      Hardfork.Chainstart,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 1150000n }),
      Hardfork.Homestead,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 1400000n }),
      Hardfork.Homestead,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 9200000n }),
      Hardfork.MuirGlacier,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 12244000n }),
      Hardfork.Berlin,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 12965000n }),
      Hardfork.London,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 13773000n }),
      Hardfork.ArrowGlacier,
      msg,
    );
    assert.equal(
      c.getHardforkBy({ blockNumber: 15050000n }),
      Hardfork.GrayGlacier,
      msg,
    );
    // merge is now specified at 15537394 in config
    assert.equal(
      c.getHardforkBy({ blockNumber: 999999999999n }),
      Hardfork.Paris,
      msg,
    );
    ...
    assert.equal(
      c.setHardforkBy({ blockNumber: 0n }),
      Hardfork.Chainstart,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 1149999n }),
      Hardfork.Chainstart,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 1150000n }),
      Hardfork.Homestead,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 1400000n }),
      Hardfork.Homestead,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 12244000n }),
      Hardfork.Berlin,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 12965000n }),
      Hardfork.London,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 13773000n }),
      Hardfork.ArrowGlacier,
      msg,
    );
    assert.equal(
      c.setHardforkBy({ blockNumber: 15050000n }),
      Hardfork.GrayGlacier,
      msg,
    );
    // merge is now specified at 15537394 in config
    assert.equal(
      c.setHardforkBy({ blockNumber: 999999999999n }),
      Hardfork.Paris,
      msg,
    );
  });
  ...
  it("should throw if no hardfork qualifies", () => {
    ...
    const c = createCustomCommon({ hardforks }, Sepolia);
    const f = () => {
      c.getHardforkBy({ blockNumber: 0n });
    };
    ...
    assert.equal(
      c.setHardforkBy({ blockNumber: 3n }),
      Hardfork.SpuriousDragon,
      msg,
    );
  });
  ...
  it("setHardfork(): hardforkChanged event", () => {
    ...
    c.events.on("hardforkChanged", (hardfork: string) => {
      assert.equal(
        hardfork,
        Hardfork.Byzantium,
        "should send correct hardforkChanged event",
      );
    });
  ...
  it("nextHardforkBlockOrTimestamp()", () => {
    ...
    msg =
      "should return correct next HF (mainnet: byzantium -> constantinople)";
    assert.equal(
      c.nextHardforkBlockOrTimestamp(Hardfork.Byzantium)!,
      BigInt(7280000),
      msg,
    );

    msg =
      "should return null if next HF is not available (mainnet: cancun -> prague)";
    ...
    msg =
      "should return null if next HF is not available (goerli: cancun -> prague)";
    ...
    assert.equal(
      c2.nextHardforkBlockOrTimestamp("petersburg")!,
      BigInt(1561651),
      msg,
    );
  });
  ...
  it("hardforkIsActiveOnBlock() / activeOnBlock()", () => {
    ...
    assert.equal(
      c.hardforkIsActiveOnBlock(Hardfork.Byzantium, 4370000),
      true,
      msg,
    );
    ...
    assert.equal(
      c.hardforkIsActiveOnBlock(Hardfork.Byzantium, 4370005),
      true,
      msg,
    );
    ...
    assert.equal(
      c.hardforkIsActiveOnBlock(Hardfork.Byzantium, 4369999),
      false,
      msg,
    );
  ...
  it("hardforkGteHardfork()", () => {
    ...
    assert.equal(
      c.hardforkGteHardfork(Hardfork.Chainstart, Hardfork.Dao),
      false,
      msg,
    );
    ...
    assert.equal(
      c.hardforkGteHardfork(Hardfork.Byzantium, Hardfork.Byzantium),
      true,
      msg,
    );
    ...
    assert.equal(
      c.hardforkGteHardfork(null, Hardfork.SpuriousDragon),
      true,
      msg,
    );
    ...
    assert.equal(
      c.hardforkGteHardfork(null, Hardfork.Constantinople),
      false,
      msg,
    );
  });
  ...
  it("_calcForkHash()", () => {
    ...
    assert.equal(
      c["_calcForkHash"](Hardfork.Chainstart, mainnetGenesisHash),
      "0xfc64ec04",
      msg,
    );
    ...
    assert.equal(
      c["_calcForkHash"](Hardfork.Homestead, mainnetGenesisHash),
      "0x97c2c34c",
      msg,
    );
    ...
    assert.equal(
      c["_calcForkHash"](Hardfork.Byzantium, mainnetGenesisHash),
      "0xa00bc324",
      msg,
    );
    ...
    for (const chain of chains) {
      ...
      for (const hf of c.hardforks()) {
        if (typeof hf.forkHash === "string") {
          ...
          assert.equal(c["_calcForkHash"](hf.name, chain[1]), hf.forkHash, msg);
        }
      }
    }
  });
  ...
  it("forkHash()", () => {
    let c = new Common({ chain: Mainnet, hardfork: Hardfork.Byzantium });
    ...
    assert.equal(
      c.forkHash(Hardfork.SpuriousDragon, genesisHash),
      "0x3edd5b10",
      msg,
    );

    c = new Common({ chain: Mainnet, hardfork: Hardfork.Shanghai });
  ...
  it("forkHash(): should not change forkHash if timestamp is at genesis timestamp", () => {
    ...
    const zeroCommonShanghaiFork = zeroCommon.forkHash(
      Hardfork.Shanghai,
      genesisHash,
    );
    const zeroCommonCancunFork = zeroCommon.forkHash(
      Hardfork.Shanghai,
      genesisHash,
    );
    ...
    const nonzeroCommonShanghaiFork = zeroCommon.forkHash(
      Hardfork.Shanghai,
      genesisHash,
    );
    const nonzeroCommonCancunFork = zeroCommon.forkHash(
      Hardfork.Shanghai,
      genesisHash,
    );
