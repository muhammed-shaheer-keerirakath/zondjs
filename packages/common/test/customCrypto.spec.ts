...
describe("[Common]: Custom Crypto", () => {
  ...
  it("keccak256", () => {
    ...
    let msg =
      "Should initialize with custom keccak256 function and use properly (main constructor)";
    assert.deepEqual(
      c.customCrypto.keccak256!(value),
      new Uint8Array([2, 1]),
      msg,
    );
    ...
    assert.deepEqual(
      c.copy().customCrypto.keccak256!(value),
      new Uint8Array([2, 1]),
      msg,
    );
    ...
    msg =
      "Should initialize with custom keccak256 function and use properly (custom() constructor)";
    assert.deepEqual(
      c.customCrypto.keccak256!(value),
      new Uint8Array([2, 1]),
      msg,
  ...
  it("sha256", () => {
    ...
    const c = new Common({ chain: Mainnet, customCrypto });
    assert.equal(
      c.customCrypto.sha256!(msg)[0],
      0xff,
      "used custom sha256 function",
    );
  });
  ...
  it("ecsign", () => {
    ...
    const c = new Common({ chain: Mainnet, customCrypto });
    assert.equal(
      c.customCrypto.ecsign!(randomBytes(32), randomBytes(32), { chainId: 0n })
        .v,
      0n,
    );
    assert.equal(
      c.customCrypto.ecsign!(randomBytes(32), randomBytes(32)).v,
      27n,
    );
  });
});
