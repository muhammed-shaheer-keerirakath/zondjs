import { unprefixedHexToBytes, utf8ToBytes } from "@theqrl/zondjs-util";
...
it(
  ...
  randomBefore((t: EciesTestContext) => {
    ...
    assert.deepEqual(
      message,
      decrypted,
      "encryptMessage -> decryptMessage should lead to same",
    );
  }),
);
...
it(
  ...
  randomBefore((t: EciesTestContext) => {
    ...
    const header = t.context.b.parseHeader(
      t.context.a.createBlockHeader(body.length) as Uint8Array,
    );
    assert.equal(
      header,
      body.length,
      "createBlockHeader -> parseHeader should lead to same",
    );

    const parsedBody = t.context.b.parseBody(
      t.context.a.createBody(body) as Uint8Array,
    );
    assert.deepEqual(
      parsedBody,
      body,
      "createBody -> parseBody should lead to same",
    );
  }),
);
