...
describe("RLPx simulator tests", () => {
  it("RLPX: add working node", async () => {
    ...
    const { rlpxs, peer } = util.initTwoPeerRLPXSetup(
      undefined,
      undefined,
      undefined,
      basePort + 1,
    ...
    await new Promise((resolve) => {
      rlpxs[0].events.on("peer:added", async (peer) => {
        ...
        assert.equal(
          rlpxs[0].getPeers().length,
          1,
          "peer list length should be 1",
        );
        assert.equal(
          rlpxs[0]._getOpenSlots(),
          9,
          "should have maxPeers - 1 open slots left",
        );
    ...
    await new Promise((resolve) => {
      rlpxs[0].events.once("peer:removed", async (_, reason: any) => {
        ...
        assert.equal(
          rlpxs[0]._getOpenSlots(),
          10,
          "should have maxPeers open slots left",
        );
        await util.delay(500);
        util.destroyRLPXs(rlpxs);
      });
