import { Common, Hardfork, Mainnet, Sepolia } from "@theqrl/zondjs-common";
          ...
          switch (code) {
            case ETH.MESSAGE_CODES.STATUS: // (-> 1)
              try {
                ...
                assert.equal(
                  err.message,
                  "NetworkId mismatch: 0xaa36a7 / 0x01",
                );
              }
              util.destroyRLPXs(rlpxs);
              resolve(undefined);
      ...
      rlpxs[1].events.on("peer:added", function (peer: Peer) {
        ...
        protocol.events.once("message", (code) => {
          switch (code) {
            case ETH.MESSAGE_CODES.STATUS:
              assert.fail(
                "should not have been able to process status message",
              );
              break;
          }
        });
