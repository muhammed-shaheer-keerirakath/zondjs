...
const PRIVATE_KEY =
  "0xd772e3d6a001a38064dd23964dd2836239fa0e6cec8b28972a87460a17210fe9";
...
dpt.events.on("peer:added", (peer) => {
  ...
  console.log(
    chalk.green(`New peer: ${info} (total: ${dpt.getPeers().length})`),
  );
});
...
dpt.events.on("peer:removed", (peer) => {
  console.log(
    chalk.yellow(
      `Remove peer: ${bytesToHex(peer.id!)} (total: ${dpt.getPeers().length})`,
    ),
  );
});
