...
const CHECK_BLOCK =
  "1638380ab737e0e916bd1c7f23bd2bab2a532e44b90047f045f262ee21c42b21";
    ...
    switch (code) {
      ...
      case devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS: {
        ...
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          ...
          eth.sendMessage(devp2p.ETH.MESSAGE_CODES.BLOCK_HEADERS, [
            payload[0],
            headers,
          ]);
        }
      ...
      case devp2p.ETH.MESSAGE_CODES.BLOCK_HEADERS: {
        if (!forkVerified) {
          ...
          const header = createBlockHeaderFromBytesArray(payload[1][0], {
            common,
          });
          if (bytesToUnprefixedHex(header.hash()) === expectedHash) {
            console.log(
              `${addr} verified to be on the same side of the ${CHECK_BLOCK_TITLE}`,
            );
...
setInterval(() => {
  ...
  const queueLength2 = rlpx["_peersQueue"].filter(
    (o) => o.ts <= Date.now(),
  ).length;
